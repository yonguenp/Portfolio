...
# 박용근 — [게임 클라이언트 개발자 포트폴리오](https://github.com/yonguenp/Portfolio/edit/main/README.md)

## 소개

아케이드부터 모바일까지, 11년간 게임 클라이언트 개발과 팀 리딩을 병행해온 개발자입니다.

홍인터내셔날에서 C++ 기반 글로벌 아케이드 머신 개발로 시작해, 하이브로에서 누적 다운로드 1,000만 이상의 라이브 게임을 기획·클라이언트·서버 구분 없이 3인 체제로 운영했습니다. 샌드박스 네트워크에서는 개발팀 창설 멤버로 합류해 10명 규모의 팀을 30명 이상으로 성장시키며 팀장으로서 공포의 술래잡기(DAU 3만), 메타토이드래곤즈(DAU 10만) 등 다수의 타이틀을 기획부터 런칭, 라이브 서비스까지 완수했습니다.

팀장으로 일하며 얻은 가장 큰 자산은 **"개발자가 무엇을 힘들어하는지"** 와 **"사업부가 무엇을 원하는지"** 를 동시에 이해하게 된 것입니다. 기획·디자인·개발 각 파트의 언어를 모두 이해하고 있어, 의견 충돌이 생기거나 프로젝트가 표류할 때 말로만 설득하는 대신 프로토타입을 빠르게 만들거나 툴을 직접 구축해 문제를 해결해왔습니다. 담당이 아닌 프로젝트가 6개월간 정체되어 있을 때 자원해서 투입되어 반년 만에 런칭까지 완수한 경험이 이를 가장 잘 보여줍니다.

클라이언트 개발자이지만 클라이언트 영역에만 머물지 않습니다. 서버(PHP/MySQL/Redis), 빌드 자동화(Jenkins/CI-CD), SDK 설계, 인프라 구축까지 직접 다뤄왔으며, 필요하다면 어느 영역이든 뛰어들어 팀 전체의 리스크를 줄이는 것을 당연하게 여깁니다. 이 경험들이 쌓여 지금은 개별 태스크가 프로젝트 전체에 미치는 영향을 항상 고려하며 개발하는 습관이 자연스럽게 몸에 배었습니다.

최근에는 생성형 AI를 게임 개발 워크플로우에 적극적으로 접목하고 있습니다. Claude Code를 주력으로 활용해 로직 설계부터 문서화까지 AI와 협업하는 방식을 직접 설계했으며, AI 멀티에이전트 완전 자동화 파이프라인 실험을 통해 기획·개발·QA 전 단계를 AI가 자율적으로 수행하는 구조를 검증하기도 했습니다. 새로운 기술을 빠르게 흡수하고 실무에 적용하는 것, 그리고 그 과정을 팀 전체의 생산성 향상으로 연결하는 것이 제가 가장 즐기는 일입니다.

**저의 강점은 세 가지입니다.**

- 🔧 **영역을 가리지 않는 실무 해결사** — 클라이언트에 머물지 않고 서버·빌드 자동화·인프라까지 직접 다뤄온 경험으로 팀의 어느 곳에서든 기여 가능
- 🤝 **협업과 조율** — 기획·디자인·개발 각 파트의 언어를 이해하고, 프로젝트가 막힌 곳에서 프로토타입과 툴로 직접 돌파구를 만들어온 경험
- 🤖 **AI 기반 개발 워크플로우** — Claude Code 기반 작업 방식을 직접 설계해 실무에 적용 중이며, AI R&D 실험을 통해 개발 가능성의 경계를 지속적으로 확장 중


| | |
|---|---|
| **주요 기술** | Unity (C#) · Cocos Creator (TypeScript) · Cocos2d-x (C++) · React · PHP · MySQL |
| **경력** | 샌드박스네트워크 · 게임 클라이언트 개발 11년 |
| **역할** | 게임 클라이언트 개발 · SDK 연동 · 서버 연동 · 웹 · PHP서버 개발 · 인게임 지표 추적 · 팀 리딩 · 프로젝트 관리 |
...
> 샌드박스네트워크 | Unity · Cocos Creator · React · PHP · MySQL | 개발 리드
...
> 샌드박스네트워크 | Unity · C# 소켓 서버 · PHP · MySQL | PM·디렉터 겸임
...
> 샌드박스네트워크 | Unity · PHP · MySQL | 클라이언트
...
> 샌드박스네트워크 | Unity · Android · iOS · Cocos Creator | 개발 리드
