...
public class MainMenuFishFarmButton : MonoBehaviour
{
    public GameObject FishfarmNoneIcon;
    public GameObject FishfarmSomeIcon;
    public GameObject FishfarmMaxIcon;
    ...
    Coroutine coroutineMainMenuFishfarmBoost = null;

    private void OnEnable()
    {
        FishfarmMaxIcon.SetActive(false);
        FishfarmSomeIcon.SetActive(false);
        Refresh();

        RefreshBooster();
    }

    public void OnClickFishfarmButton()
    {
        ...
        NetworkManager.GetInstance().SendApiRequest("plant", 3, data, (response) =>
        {
            ...
            ShowFishfarmRewardPopup(rewards);
        });
    }
    ...
    public void Refresh()
    {
        ...
        neco_data.GAIN_STATE state = neco_data.Instance.CurGoldState();
        switch (state)
        {
            case neco_data.GAIN_STATE.NONE:
                if (FishfarmNoneIcon.activeSelf == false)
                {
                    FishfarmNoneIcon.SetActive(true);
                    FishfarmNoneIcon.GetComponent<Animation>().Play("Fishfarm_idle");
                }
                FishfarmSomeIcon.SetActive(false);
                FishfarmMaxIcon.SetActive(false);
                break;
            case neco_data.GAIN_STATE.SOME:
                if (FishfarmSomeIcon.activeSelf == false)
                {
                    FishfarmSomeIcon.SetActive(true);
                    FishfarmSomeIcon.GetComponent<Animation>().Play("Fishfarm_max");
                }
                FishfarmNoneIcon.SetActive(false);
                FishfarmMaxIcon.SetActive(false);
                break;
            case neco_data.GAIN_STATE.FULL:
                if (FishfarmMaxIcon.activeSelf == false)
                {
                    FishfarmMaxIcon.SetActive(true);
                    FishfarmMaxIcon.GetComponent<Animation>().Play("Fishfarm_Full");
                }
                FishfarmNoneIcon.SetActive(false);
                FishfarmSomeIcon.SetActive(false);
                break;
        ...
        {
            uint next = neco_data.Instance.NextGoldUpdate();
            if (next > 0)
    ...
    void RefreshBooster()
    {
        ...
        boosterObject.SetActive(neco_data.Instance.GetFarmBoostTime() > NecoCanvas.GetCurTime());
        boosterArrowObject.SetActive(neco_data.Instance.GetFarmBoostTime() > NecoCanvas.GetCurTime());
    }

    void RefreshFishfarmBoostTimeData()
    {
        if (coroutineMainMenuFishfarmBoost != null)
        {
            StopCoroutine(coroutineMainMenuFishfarmBoost);
        }

        coroutineMainMenuFishfarmBoost = StartCoroutine(RefreshFishfarmBoostTime());
    }
    ...
    void ShowFishfarmRewardPopup(List<RewardData> rewardList)
    {   
        NecoCanvas.GetPopupCanvas().OnFishFarmPopupShow(rewardList, SUPPLY_UI_TYPE.FISH_FARM);
    }

    bool CheckPrologue()
    {
        return neco_data.GetPrologueSeq() < neco_data.PrologueSeq.양어장닫고대사;
    }
