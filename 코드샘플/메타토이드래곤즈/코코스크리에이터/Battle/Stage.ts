...
import { _decorator, Component, Node, instantiate, Prefab, director, Label, color, Color, equals, ProgressBar, sys } from 'cc';
...
import { BattleAIMonsterData, BattleDragonData, BattleMonsterBaseData } from '../Character/BattleData';
...
import { GetChild, RandomInt, StringBuilder } from '../Tools/SandboxTools';
import { User, UserDragon } from '../User/User';
import { BattleUI } from './BattleUI';
import { BattleSimulatorResourceManager } from './Battle_simulator/BattleSimulatorResourceManager';
 ...
 * Name = Stage
 * DateTime = Mon Feb 21 2022 15:38:37 GMT+0900 (대한민국 표준시)
 * Author = ahnhyeon5017
 * FileBasename = Stage.ts
 * FileBasenameNoExtension = Stage
 * URL = db://assets/Scripts/Battle/Stage.ts
 * ManualUrl = https://docs.cocos.com/creator/3.3/manual/en/
 *
 */

export enum eBattleState {
    None = 0,
    Playing = 1,
    Win = 2,
    Lose = 3,
...
export class StageData {
    ...
    private levelUpDragonList: {} = null;
    public get LevelUpDragonList(): {} {
        return this.levelUpDragonList;
    }
    private newRecord: number = -1;
    public get NewRecord(): number {
    ...
    Init(mapNode: Node, stageData: any): void {
        ...
        this.rewards['deserveReward'] = false;
        ...
        let ones = mapNode.getChildByName('Ones');
        if(ones != null) {
            this.ones = ones;
        }
        ...
        if(User.Instance.UNO > 0)
        {
            ...
            this.StageSpeed = BattleSpeedValue[bSpeed];
        }else{
            let speed = stageData['jsonData']['battleSpeed'];
    ...
    CheckAndCreateUserDragon(player : any , x, y , arrayCount)
    {
        ...
        if(dragonData != null) 
        {
            if(User.Instance.UNO > 0){
                ...
                this.CreateSimulatorDragon(player['dtag'], player['btag'], x, y, arrayCount, dragonData.Level, dragonData.SLevel);
            }
    ...
    CreateDragon(dtag: number, btag: number, x: number, y: number, yMax: number, level: number = 1, skillLevel: number = 1) {
        ...
        let clone :Node = instantiate(dragonData.Prefab);
        ...
        if(battleDragon != null) {
            battleDragon.Init();
            battleDragon.Set(battleDragonData);
    ...
    CreateSimulatorDragon(dtag: number, btag: number, x: number, y: number, yMax: number, level: number = 1, skillLevel: number = 1)
    {
        let baseData = this.dataTable.Get(dtag);
        let dragonData = User.Instance.DragonData.GetDragon(dtag);
        let statData: { HP : number, ATK : number, DEF : number, CRI : number } = null;
    ...
    SortOrderCharacter() {
        ...
        this.charaters.forEach((element, index) => {
            ...
            if(element.parent == this.orderNode[index]) {
                return;
            }
                ...
                for(var y = 0 ; y < arrayCount ; y++) {
                    ...
                    if(data['spawn'] != undefined) {
                        ...
                        if(User.Instance.UNO > 0)
                        {
                            ...
                            let prefab = BattleSimulatorResourceManager.Instance.GetResource(ResourcesType.MONSTER_PREFAB)[targetMonsterData.IMAGE];
                            this.CreateSimulatorMonster(x, y, arrayCount, data['btag'], targetMonsterData, targetSpawnData, prefab);
                        }
    ...
    CreateSimulatorMonster(x: number, y: number, yMax: number, btag: number, targetMonsterData: MonsterBaseData, targetSpawnData: MonsterSpawnData, randomPrefab: Prefab){
        let clone: Node = instantiate(randomPrefab);
        let battleMonster = clone.getComponent(BattleAIMonster);
        let battleAIMonsterData = new BattleAIMonsterData();
        battleAIMonsterData.SpawnData = targetSpawnData;
        ...
        if(battleMonster != null) {
            ...
            // let hpBarNode = battleMonster.node.getChildByName('character_hp_red');
            // if(hpBarNode == null)
            // {
            //     let hp_bar = BattleSimulatorResourceManager.Instance.GetResourceByName('character_hp_red');
            //     let hp_bar_node = instantiate(hp_bar);
            //     battleMonster.node.addChild(hp_bar_node);
            //     let bar_node = hp_bar_node.getChildByName('bar');
            //     if(bar_node != null) {
            //         battleMonster.Hp_bar = bar_node.getComponent(ProgressBar);
            //     }
            // }
    ...
    MoveRight(dt: number) {
        ...
        if(this.ones != null) {
            let position = this.ones.getPosition();
            position.x -= pos2;
            this.ones.setPosition(position);
        }
    ...
    UpdateCharacter(dt: number) {
        ...
        this.SortOrderCharacter();
    }

    UpdateTime(dt: number) {
        this.time -= dt;
        BattleUI.BattleTime(Math.round(this.time+0.5));
    ...
    SetData(jsonData: any) {
        ...
        if(jsonData['enemy'] != undefined) {
            ...
            if(User.Instance.UNO < 0)
            {
                ...
                const enemyCount = enemy.length;//웨이브 인덱스만 넘어옴 - 인덱스 끼리 묶어서 2차배열화, btag type은 아무거나 넣기

                let tempdata = [[],[],[]];
                for(let i = 0 ; i < enemyCount ; i++) {
                    let position =this.monsterSpawnTable.GetKey(enemy[i]).POSITION;//포지션 기준으로 세팅
                    let row = {};
                    row['btag'] = 1;
                    row['type'] = 1;
                    row['spawn'] = this.monsterSpawnTable.GetKey(enemy[i]);
                    tempdata[position - 1].push(row);
                }
                this.curWaves['wave'] = tempdata;
            }
            else
            {
                let enemy = jsonData['enemy'];
        ...
        if(jsonData['levelup'] != undefined)
        {
            let arrayLength = jsonData['levelup'].length;
            if(jsonData['levelup'] != null && jsonData['levelup'] != undefined && arrayLength > 0)
            {
                for(let j = 0 ; j < arrayLength ; j++) {
                    let dragonTag = jsonData['levelup'][j];
                    this.levelUpDragonList[dragonTag] = dragonTag;
                }
            }
        }

        if(jsonData['nr'] != undefined) {
            this.newRecord = jsonData['nr'];
        }
    }
    ...
    SetDamage(dragonTag : number, damage : number)
    {
        let keys = Object.keys(this.DamageStatistic);
        if(keys == null || keys.length <= 0)
        {
            this.DamageStatistic[dragonTag] = damage;
        }
        else{
            let keyCheck = keys.filter(element=> element == (dragonTag.toString()));
            if(keyCheck.length > 0)
            {
                let tempDamage = this.DamageStatistic[dragonTag];
                this.DamageStatistic[dragonTag] = tempDamage + damage;
            }else{
                this.DamageStatistic[dragonTag] = damage;
            }
        }
    ...
    AddDamageStatistics()
    {
        let data : any = DataManager.GetData("AdventureDamageStatistic");
        if(data == null || data == undefined){
            DataManager.AddData("AdventureDamageStatistic" , this.DamageStatistic);
        }
        else//겹치는 데이터 정제하기
        {
            let tempAdventureWaveDamage : any = {};
            let prevDataKeys = Object.keys(data);//기존 데이터 복사
            prevDataKeys.forEach(key=>{
                tempAdventureWaveDamage[key] = data[key];
            });

            let currentDataKeys = Object.keys(this.DamageStatistic);
            currentDataKeys.forEach(currentKey =>{
                ...
                {
                    let tempRewardData = tempAdventureWaveDamage[currentKey];
                    let currentData = this.DamageStatistic[currentKey];

                    tempAdventureWaveDamage[currentKey] = tempRewardData + currentData;
                }
                else//신규
                {
                    tempAdventureWaveDamage[currentKey] = this.DamageStatistic[currentKey];
                }
            });

            DataManager.DelData("AdventureDamageStatistic");
            DataManager.AddData("AdventureDamageStatistic" , tempAdventureWaveDamage);
        }
    }

    public GetNumDragonAlive()
    {
        const len = this.dragons.length;
        let numAlive = 0;
        for (var i = 0; len > i; ++i)
        {
            let dragon = this.dragons[i];
            if (dragon && !dragon.Death) {
                ++numAlive;
            }
        }
...
@ccclass('Stage')
export class Stage extends Component implements EventListener<WaveEvent> {
    private stateMachine: WaveStateMachine = null;
    private gameManager: GameManager = null;
    private stageData:StageData = null;
    public get StageData(): StageData {return this.stageData;}
        ...
        if(!this.pause) {
            ...
            switch(this.stageData.BattleState) {
                ...
                case eBattleState.Win: {//this.stageData.Rewards에 리워드 정보 있음 { star:스타 갯수, rewards:아이템들{type:rewardType,no:번호(아이템 등),count:갯수} }.
                    ...
                    rewards.deserveReward = true;

                    this.stageData.Rewards['levelup'] = this.stageData.LevelUpDragonList;
                    DataManager.AddData("StageResultData", this.stageData.Rewards);
                    SoundMixer.DestroyAllClip();
                    SceneManager.SceneChange("battleReward");

                    // 튜토리얼 관련
                    DataManager.AddData("StageTutorialData", this.stageData);
                } break;
                ...
                case eBattleState.TimeOver: {//패배시
                    DataManager.AddData("StageResultData", this.stageData.Rewards);
                    SoundMixer.DestroyAllClip();
                    SceneManager.SceneChange("battleReward");
                    this.pause = true;
    ...
    OnEvent(eventType: WaveEvent): void {
        ...
        switch(eventType.Data['state']) {
            case 'Init': {
                ...
                if(!this.isSimulator()){
                    BattleUI.init(this.stageData, eventType.Data['stageData']);
                }else{
                ...
                if(DataManager.GetData("AdventureDamageStatistic") != null){
                    DataManager.DelData("AdventureDamageStatistic");
                }
            } break;
            case 'datas': {
            ...
            case 'WaveMove': {
                BattleUI.SetWaveLabel(StringBuilder("{0}/{1}", this.stageData.CurWave, this.stageData.MaxWave))
                this.stateMachine.Change(eventType.Data['state']);
            } break;
            case 'WaveBattle': {
                this.stateMachine.Change(eventType.Data['state']);
