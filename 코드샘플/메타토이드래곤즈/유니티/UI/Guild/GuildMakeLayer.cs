...
{
    public class GuildMakeLayer : TabLayer
    {
        ...
        eGoodType needMoneyType
        {
            get
            {
                return SBFunc.ConvertStringToItemType(GameConfigTable.GetConfigValue("GUILD_ESTABLISH_COST_TYPE", "GOLD"));
            }
        }
        int curEmblem = 1;
        int curMark = 1;
        ...
        public void OnClickMakeGuild()
        {
            ...
            string name = guildNameInputField.text;
            string info = guildInfoInputField.text;
            if (string.IsNullOrEmpty(name))
                return;
