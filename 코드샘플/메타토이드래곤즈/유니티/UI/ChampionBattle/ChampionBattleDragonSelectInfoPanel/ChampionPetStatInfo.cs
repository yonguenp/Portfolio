...
public class ChampionPetStatInfo : PetStatInfo
{
    [SerializeField]
    ...
    {
        return parentPopup.UserDragonData;
    }
    ...
    {
        var currentDragonData = GetDragonInfo().GetDragon(CurPopupDragonTag) as ChampionDragon;
    ...
    protected override void RefreshCurrentPetData()
    {
        ...
        var currentDragonData = GetDragonInfo().GetDragon(CurPopupDragonTag) as ChampionDragon;
        if (currentDragonData == null) return;

        var userPetInfo = currentDragonData.ChampionPet;
        if (userPetInfo == null || userPetInfo.Tag != CurPopupPetTag)
        {
            userPetInfo = GetPetInfo().GetPet(CurPopupPetTag) as ChampionPet;
        }
        if (userPetInfo == null) return;
