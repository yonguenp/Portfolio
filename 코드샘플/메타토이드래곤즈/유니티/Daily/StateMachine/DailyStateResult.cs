...
{
    public class DailyStateResult : DailyState
    {
...
#endif
                NetworkManager.Send("daily/dailyresult", form, (NetworkManager.SuccessCallback)((jsonData) =>
                { //성공 결과에 따라 처리
                    ...
                    && jsonData["rs"].Value<int>() == (int)eApiResCode.OK)
                    {
                        ...
                        {
                            switch ((eBattleState)jsonData["state"].Value<int>())
                            {
                                case eBattleState.Playing:
                                    DailyManager.Instance.SetWaveData(jsonData);
                                    if (DailyManager.Instance.IsWaveCheck())
                                    {
                                        BossAlert();
                                        IsNetworkWait = false;
                                    }
                                ...
                                case eBattleState.TimeOver:
                                    ...
                                    if (DailyManager.Instance.IsRewardCheck())
                                    {
                                        IsNetworkWait = false;
                                    }
                                    else
