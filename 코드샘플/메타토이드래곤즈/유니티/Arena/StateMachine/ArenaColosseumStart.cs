...
{
    public class ArenaColosseumStart : ArenaColosseumState, EventListener<UIBattleObjectEndEvent>
    {
        bool animEnd = false;
            ...
            if (base.OnEnter()) //처음 시간설정, UI, 연출 딜레이 등
            {
                ...
                if (offenseDragons != null)
                {
                    foreach (var it in offenseDragons)
                    {
                        ...
                        var arenaDragon = Stage.GetOffenseSpine(it.Value);
                        if (arenaDragon == null)
                            continue;

                        ProCamera2D.Instance.AddCameraTarget(arenaDragon.transform, 0, 0);
                ...
                if (defenseDragons != null)
                {
                    foreach (var it in defenseDragons)
                    {
                        ...
                        var arenaDragon = Stage.GetDefenseSpine(it.Value);
                        if (arenaDragon == null)
                            continue;
                ...
                ArenaManager.Instance.ColosseumData.InitializeCharacterPos();
