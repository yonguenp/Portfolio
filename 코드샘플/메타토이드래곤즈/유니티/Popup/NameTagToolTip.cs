...
namespace SandboxNetwork
{
    ...
    /// </summary>
    public class NameTagToolTip : ToolTip
    {
        #region OpenPopup
        public static NameTagToolTip OpenPopup(string _title, GameObject _targetParent)
        {
            return OpenPopup(new TooltipPopupData(new ToolTipData(_title, "", _targetParent)));
        }
        public static NameTagToolTip OpenPopup(TooltipPopupData data)
        {
            if (data == null)
                return null;

            return PopupManager.OpenPopup<NameTagToolTip>(data);
        }
        #endregion
