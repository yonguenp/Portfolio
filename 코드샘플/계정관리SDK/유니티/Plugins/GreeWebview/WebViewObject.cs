...
public class WebViewObject : MonoBehaviour
{
...
#if WEBVIEW_OFFSET_ADJUST
            int keyboardHeight = AdjustBottomMargin(mMarginBottom);
            int screenHeight = Display.main.systemHeight;
            EvaluateJS($"window.Samanda.onKeyboardVisibility({keyboardHeight}, {screenHeight})");
#else
            SetMargins(mMarginLeft, mMarginTop, mMarginRight, mMarginBottom);
#endif // WEBVIEW_OFFSET_ADJUST
        }

        SamandaLauncher.onKeyboardVisible.Invoke();
    }
    ...
    public int AdjustBottomMargin(int bottom)
    {
        ...
        {
#endif // WEBVIEW_OFFSET_ADJUST
        int keyboardHeight = SamandaLauncher.GetKeyboardHeight(true);
#if WEBVIEW_OFFSET_ADJUST
        return (bottom > keyboardHeight) ? bottom : keyboardHeight - bottom;
#else
