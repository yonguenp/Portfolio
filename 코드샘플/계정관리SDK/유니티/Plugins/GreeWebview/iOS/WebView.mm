...
- (void)setMargins:(int)left top:(int)top right:(int)right bottom:(int)bottom
{
	...
	UIEdgeInsets safeInset = view.safeAreaInsets;
	screen.origin.y -= safeInset.top;
	screen.size.height += safeInset.top + safeInset.bottom;
	screen.origin.x -= safeInset.left;
	screen.size.width += safeInset.left + safeInset.right;
