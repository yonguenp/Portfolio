...
> 두 번의 별도 `eval` 호출로 나눠서야 정상값을 볼 수 있었다. 실제
> 게임에서는 이 싱글턴이 세션 초반에(첫 비상/완성 이펙트 등에서) 이미
> 만들어져 여러 프레임이 지난 뒤에 흔들기가 발동하므로 문제가 안 되지만,
> **이 컴포넌트를 새로 다루는 코드에서 인스턴스화 직후 곧바로 레이아웃
> 값을 읽으면 NaN을 만날 수 있다는 걸 기억해 둘 것.**
