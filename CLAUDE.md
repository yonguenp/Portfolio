...
### 후속 2 — "svg 이펙트 이미지와 텍스트 겹침" (같은 날)

박스가 작다 보니(400×165) 카드와 "흔듬" 텍스트가 같은 영역 전체를 각자
차지해서 시각적으로 겹쳤다 — 화면 전체를 쓰는 완성/비상 이펙트와 달리
이 작은 박스는 여유가 없어서 그대로 옮기면 안 됐다. 박스를 상단 텍스트
띠(전체 높이의 24%)와 하단 카드 영역으로 나눠서 서로 안 겹치게 했다.
라벨 폰트도 40→26으로 줄여 좁아진 띠 안에 맞췄다. 검증(라이브): 같은
세션의 두 번째 트리거(레이아웃이 이미 유효한 상태)로 `labelBottom=279.6,
rowTop=279.6`(간격 사실상 0, 겹침 없이 정확히 맞닿음) 확인.
