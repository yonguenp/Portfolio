...
이 항목은 2026-08-19~22 사이 진행된 대규모 리팩터 요청(GoStop UI 분리,
캡처/뒷패 타이밍 재정렬, 9월 열끗 팝업 타이밍, 싹쓸이 규칙 조사,
`ScoreDetailPopup`/`BadgeStripArea` 위치 수정, 비상 시스템 4종)의 일부다 —
남은 항목(Player 상태창 Prefab화, 결과 화면 자금 상세 보강, 코드-UI의
광범위한 Prefab 전환)은 계속 진행 중.
