...
## 고스톱 — 광박 배지 오탐 조사 (재현 실패, 원인 미확정) (2026-09-04)

"광박 대상이아닌데 광박 아이콘에 불이들어오네" 신고 — 조사했지만 **재현에
실패했다.** 아래는 시도한 검증과 결과를 정직하게 남긴다(다음에 이어서
조사할 때 같은 걸 다시 안 해도 되게):

1. `GoStopRules.IsLiveGwangBakRisk`(순수 함수, `mine.Count(Gwang)>0 →
   false, 아니면 others.Any(o.Count(Gwang)>=3)`)를 직접 호출해 3가지
   케이스(내가 1장+상대 3장→False, 내가 0장+상대들 1+2장(누구도 3장
   미달)→False, 내가 0장+상대 3장→True) 전부 정확한 값을 돌려주는 것
   확인 — 로직 자체는 이상 없다.
2. `DrawBadgeStrip`(실제 배지에 색을 입히는 함수)을 4인 라이브 게임에서
   30턴 자연 진행시키며(참가선언·필드선택·9월열끗 팝업 자동 응답,
   `NewGame()` 경계 1회 포함) **매 턴마다** 활성 좌석 전부에 대해
   `IsLiveGwangBakRisk`로 독립 재계산한 기대값과 실제
   `riskIconBg[0].color`(GwangBakColor와 픽셀 비교)를 대조하는 차등
   테스트를 돌렸다 — **30턴 내내 불일치 0건.**
3. 배지 3개(광/멍/피)의 실제 월드 좌표(`GetWorldCorners`)를 재서 서로
   겹치지 않고 순서대로 배치돼 있는 것도 확인 — "선(先) 배지나 다른
   배지가 겹쳐 보여서 광박으로 오인"이라는 가설도 배제됐다.
4. `SetRisk` 호출부는 `DrawBadgeStrip` 하나뿐(`HideAllBadges`의 초기화
   호출 제외) — 다른 곳에서 스킨 값이 새어 들어오는 경로도 없었다.

**결론 — 이번 세션에서 가진 정보로는 원인을 못 찾았다.** 순수 로직·
차등 라이브 테스트·레이아웃 겹침까지 확인했는데도 재현이 안 된 이상,
다음에 이 버그가 다시 보이면 **정확한 재현 조건**(몇 인 게임인지, 그
좌석이 실제로 광 몇 장을 갖고 있었는지, 다른 좌석들 광 장수, 혹시
직전에 총통·쓰리뻑·나가리 같은 특수 종료가 있었는지, 스크린샷)이
있어야 다시 조사할 수 있다.
