...
### 도움말 버튼(`HelpBtn`) — 씬 오브젝트로 승격 (2026-09-06)

"나가기버튼 옆에 도움말 버튼이 또 코드로생성되나봄 씬 하이어라키안에
배치해줄것" — `ExitBtn`은 이미 씬 오브젝트였는데(2026-09-01) 나중에
추가된 `HelpBtn`(2026-09-05)만 그 승격을 안 거쳐서 매번 코드로 새로
생성되고 있었다. `PointPriceInfo`와 같은 절차(Play 모드에서 코드가
만드는 실제 값을 리플렉션으로 읽고 Edit 모드에서 동일하게 재현)로
`ExitBtn` 바로 옆(`anchoredPos=(145,-15)`, Blue 액센트)에 구워 저장했다.
검증: 재사용 시 중복 생성 없음(1개), 클릭 시 `ShowHelp()`가 정상 호출돼
도움말 패널이 뜨는 것까지 확인.

### 게임 시작 시 "OOO님이 입장하셨습니다" 시스템 메시지 (2026-09-06)

"맨처음 게임시작할때 '~님이 입장하셨습니다'로 함께 플레이할 CPU나
플레이어 알려주기" — 오프라인은 `Start()`에서 캐릭터를 뽑은 직후,
네트워크는 호스트가 로비 인원을 확정한 직후, 나를 제외한 참가자 전원을
채팅 로그에 알린다. 채팅 패널이 아직 안 지어진 시점에 불러도
`chatEntries`에 먼저 쌓이고 `BuildStaticUI`가 곧이어 `BuildChatUI`에서
`RedrawChatLog`를 부를 때 정상 표시되므로 순서 걱정이 없다. 네트워크는
호스트에서 한 번만 불러 `AppendChatLine`의 기존 브로드캐스트로 게스트
에게도 전달한다(게스트는 이 `Start()` 분기 자체를 안 타 중복 없음).
검증: 사용자의 실제 플레이 세션 채팅 로그에서 정상 동작 확인.

### 버그 — 채팅 로그에 내 이름이 "AI"로 뜸 (2026-09-06)

"채팅 로그에 내 이름이 'AI'라고 뜸. 스테이터스박스엔 내이름이 '나'라고
뜸." — `AppendChatLine` 등 전역 방송용 호출은 `SeatNameFor(seat, -1)`로
부른다(네트워크에서 같은 문구를 여러 사람이 보므로 아무도 "나"로 안
보여야 한다는 의도로 `viewerSeat=-1`을 쓴다). 그런데 `seat == viewerSeat`
검사가 `seat=0`(PLAYER_SEAT)과 `viewerSeat=-1`을 다르다고 보고 통과시켜,
`seatCharName[PLAYER_SEAT]`가 애초에 채워진 적 없는 빈 문자열이라 맨
아래 "AI" 기본값으로 떨어졌다 — 상태박스는 `SeatName(seat)`
(`viewerSeat=PLAYER_SEAT`)을 써서 이 경로를 안 타므로 "나"가 정상
표시되는 것과 대비돼 불일치가 두드러졌다. 오프라인(`!isNetworkHost &&
!isNetworkGuest`)이면서 `seat==PLAYER_SEAT`인 경우에만 예외적으로 "나"를
반환하도록 고쳤다 — 오프라인은 로컬 뷰어가 항상 나 하나뿐이라 "나"라고
표시해도 아무도 헷갈릴 사람이 없고, 네트워크는 이 예외를 안 타 기존
"실제 정체를 그대로 보여준다"는 의도가 그대로 유지된다. 검증: 같은
좌석·같은 seat에 대해 `SeatNameFor(0,-1)`→"나"(수정 전 "AI"),
`SeatNameFor(0,0)`→"나"(회귀 없음), `SeatNameFor(1,-1)`→AI 캐릭터
실제 이름(회귀 없음) 셋을 동시에 확인했다.

## 고스톱 — 카드 애니메이션 잔여 끊김 3건 (손패 턴 토글, 조커 hand→cap,
겹친 패 툴팁 입력 방식) (2026-09-06)

"모든 카드는 툭툭끊기게 움직이는거 없애줘. 손패가 -50에 있다가 0으로
올라올때도 트윈, 보너스패가 손패→cap 이동할때도 트윈. 그것 말고도 더
찾아서 수정해달라"는 요청 + 뒤이어 "겹친 패 툴팁이 안 없어진다"는 신고,
그리고 진행 도중 "툴팁을 호버가 아니라 마우스다운/터치다운으로 바꿔달라"는
방향 정정까지 한 세션에서 처리했다.

**손패 턴 토글(-50↔0/34) 끊김 — `handFlyFrom` 신설.** `RebuildUI()`가
`handArea`를 `ClearChildren`하기 직전, 지금 손패에 있는 카드마다
`FindHandSlot(c)`로 실제 렌더 위치를 스냅샷해 `handFlyFrom[c]`에 담아둔다
(필드/Cap 캡처 때 쓰는 `flyFrom`과 완전히 같은 원리 — "지우기 전에 어디
있었는지 먼저 기억해 둔다"). `DrawPlayerHand()`가 카드를 새로 만들 때마다
`handFlyFrom`에 그 카드의 이전 위치가 있으면(그리고 실제로 거리가 있으면)
`SlideHandCard`(부드러운 smoothstep 이동, 캡처 비행과 달리 임팩트
플래시·펀치 스케일 없음 — "잡았다 놓는" 사건이 아니라 손 안에서 재배치될
뿐이므로) 코루틴으로 이어붙인다. 이 스냅샷을 턴 토글 전용으로 좁히지 않고
**손패에 있는 카드 전부**에 대해 매번 찍어두므로, 카드를 내서 나머지가
당겨져 재배치되는 일반적인 "손패 reflow"도 같은 메커니즘으로 자동
부드러워진다 — 별도 분기가 필요 없었다.

**조커(보너스패) 손패→Cap 이동 끊김 — 원인은 애니메이션 부재가 아니라
"손패 잔상"이었다.** `PlayJokerFromHandSeq`(손패에서 조커를 직접 낼 때)는
이미 `flyFrom[joker]`/`flyFromSize[joker]`를 등록하고 있어서 Cap 쪽
착지 애니메이션(`SlamToCap`) 자체는 정상 작동 중이었다 — 그런데 이전
세션에 "손패에서 필드로 나갈 때 카드가 안 사라지고 필드에 생겨서
어색하다"는 신고로 만든 `HideHandCardVisual(seat, card)`(고스트가 뜨는
즉시 손패 원본 GameObject를 `SetActive(false)`) 호출이 `PlaySeq`의
일반 매칭 경로 2곳에만 있고 `PlayJokerFromHandSeq`엔 빠져 있었다 —
`RebuildUI()`가 정식으로 지울 때까지(`Destroy` 지연 실행) 옛 손패
카드가 그대로 남아, Cap으로 날아가는 새 고스트와 잠깐 겹쳐 보이는 게
"끊긴다"는 체감의 정체였다. `h.Remove(joker)` 직전에 같은
`HideHandCardVisual(seat, joker)` 호출 한 줄을 추가해 고쳤다.

**겹친 패 툴팁 — 두 차례 방향 정정.** 1차: "터치나 마우스오버시 떴다가
마우스오버해제·터치아웃 때 사라지는" 진짜 호버로(Down/Up→Enter/Exit).
2차(같은 세션, 사용자가 곧바로 재정정): "마우스다운일 때 띄우고
마우스업이면 닫고, 터치도 다운/업으로" — 다시 press-and-hold(Down/Up)로
되돌렸다. `GoStopStackHoverTrigger`를 `IPointerDownHandler`/
`IPointerUpHandler`(+안전망으로 `IPointerExitHandler`도 Hide에 남겨둠 —
손가락이 트리거 밖으로 나간 채 뗄 때를 대비)로 최종 확정했다. Unity
UGUI는 마우스 클릭과 터치 시작/종료를 동일한 PointerDown/Up 이벤트로
합성해주므로 입력 방식을 따로 분기할 필요가 없다.

**검증(Play 모드 라이브, 리플렉션).**
- 손패 토글: 상대 턴으로 강제 전환 후 `RebuildUI()`(별도 exec 호출로
  Destroy 지연 실행 함정을 피해 프레임 경계를 확실히 둠) → 카드가
  정착된 -50 상태 확인. 내 턴으로 복귀 + `RebuildUI()` **같은 동기
  호출 안에서** 곧바로 위치 재확인 — 이번엔 이름 기반 `Find()`가 아니라
  **sibling index가 가장 큰(=방금 새로 생성된) 동일 이름 카드**를 직접
  골라(이름만으로 찾으면 파괴 대기 중인 옛 카드와 새 카드가 한 프레임
  동시에 존재해 헷갈린다는 걸 이번에 새로 확인) `GetInstanceID()`로
  서로 다른 객체임을 확인 + `.position`이 목표(약 50~84 유닛 차이)가
  아니라 이전 위치에서 겨우 0.06 유닛 떨어진 지점(=코루틴의 smoothstep
  이징이 방금 시작된 상태)에 있는 것을 확인 — 순간이동이 아니라 실제로
  보간 중임을 입증했다.
- 조커: 자연 딜에 있던 `Joker_2`를 실제 `OnPlayerPlay`로 재생 —
  동일 동기 구간에서 손패 GameObject가 `activeSelf=False`로 즉시
  전환(수정 전엔 그대로 `True`였을 지점), 이후 `captured[0]`에 정상
  캡처되고 Cap 존에 조커가 잔여물 없이 자리 잡는 것까지 확인.
- 툴팁: 자연 딜에 있던 6월 2장 겹침 슬롯의 트리거에 `OnPointerDown`을
  직접 호출 → 딤 패널 `activeSelf=True`, 이어서 `OnPointerUp` → `False`
  로 정확히 열리고 닫히는 것 확인.
...
## 고스톱 — 조커 파킹이 "영원히 완성 불가능한" 무더기를 만드는 치명적 버그
(2026-09-06)

바로 전 세션에서 만든 couldBePpeok+조커 파킹 기능을 사용자가 실전에서
바로 발견한 진짜 버그. "필드엔 9월·7월 있던 상황, 아귀가 9월패를 냄,
뒷패로 보너스패가 나옴(9월패 위에 스택됨), 다음 뒷패로 7월이 나옴 →
7월은 아귀가 가져갔는데 9월은 조커랑 같이 그냥 필드에 남아있음"이라는
신고를 받아 라이브 에디터를 직접 리플렉션으로 조사했다.

**실제 상태 확인 결과** — 필드에 `September_Tane/September_Tanzaku/
Joker_2` 3장이 슬롯 4에 함께 쌓여 있었고, `ppeokBonusPi[9]=Joker_2`로
정상 등록까지 돼 있었다(파킹 로직 자체는 의도대로 작동). 그런데
`captured[1]`(정마담)이 이미 `September_Kasu_1/2`를 갖고 있었다 — **9월의
실카드 4장 중 2장이 이미 이전 어느 시점(이번 이벤트와 무관하게)에 다른
좌석에게 캡처된 상태였던 것.** 9월은 표준 화투 구성상 정확히 4장뿐이라,
필드에 쌓인 2장(2인 파킹 페어) + 이미 캡처된 2장 = 4장 전부 계산이
끝나버려서 **세상에 더 이상 존재하지 않는 "9월 4번째 카드"를 영원히
기다리는 죽은 무더기**가 된 것이었다 — 정확히 "치명적 버그"라고 부를
만한 사고.

**왜 위험한 조건인가.** 일반적인(조커 없는) 뻑 형성은 실카드 3장이
동시에 모여야 하므로, 그 시점에 이미 캡처된 카드가 최대 1장이어야만
성립할 수 있다(4장 중 3장이 지금 막 모이는 거니까). 반면 조커 파킹은
**실카드 2장만 모이면** 트리거된다 — 이미 캡처된 카드가 2장이어도
성립 조건 자체는 만족해버려서, "죽은 무더기"가 만들어질 확률이
일반 뻑보다 훨씬 높았다(게임 중반 이후 흔히 벌어지는 "그 달 카드
일부가 이미 여기저기 흩어져 캡처된" 상태와 정확히 충돌한다).

**고침.** 파킹 직전에 "이 달의 카드가 이미 어딘가의 획득패에 몇 장
들어가 있는지" 전 좌석에 걸쳐 합산(`captured.Sum(cap =>
cap.Count(c => c.month == card.month))`)해서, **2장 이상이면 파킹을
하지 않고**(4-2(이번 페어)-2(이미 캡처)=0, 4번째 카드가 나올 수
없으므로) 그냥 원래(조커 등장 전) 흐름대로 정상 2장 캡처로 진행하게
했다. 1장 이하면(4-2-1=1, 아직 4번째가 세상에 남아있음) 기존 파킹
로직 그대로 유지.

**검증(라이브 리플렉션, 4인 게임).**
- **부정 케이스(버그 재현 조건)**: seat1에 9월 카드 2장을 미리 캡처된
  상태로 세팅 → 필드 9월 1장 + 손패 매칭 9월 1장 → 조커를 다음 뒷패로
  리깅 → 재생 → 결과: `fieldHasSepOrJoker=False, ppeokBonusPiHas9=False`,
  카드들이 즉시 정상 캡처로 처리됨(파킹 없이 진행) — 죽은 무더기가
  더 이상 안 생김.
- **긍정 케이스(정상 파킹, 회귀 확인)**: 아무도 3월 카드를 캡처한 적
  없는 깨끗한 상태에서 같은 시나리오 재현 → 조커가 정상적으로 파킹됨을
  확인한 뒤, 자연 진행 몇 초를 그대로 뒀더니 seat2가 실제로 3월 4번째
  카드를 완성해 무더기 전체(조커 포함)를 정상적으로 스윕해 가는 것까지
  확인 — 전 세션에 만든 파킹 기능 자체는 여전히 정상 작동.
- 콘솔 에러 0건.

### 딜링 시점 재확인 — 손 안 댐(전 세션에 이미 정상 확인됨)

이번 신고에서 언급된 "딜링 시점에 보너스패가 특정 카드 위에 깔린다"는
부분은 바로 전 세션에서 이미 조사해 정상 작동을 확인한 부분이고, 이번
"9월 무더기가 죽는다"는 신고는 실제로는 **딜링이 아니라 위 조커 파킹
버그**였음이 라이브 조사로 명확히 확인됐다(정마담의 9월 2장은 게임
진행 중 정상적인 캡처를 통해 축적된 것이지 딜링 시점 문제와 무관) —
딜링 코드는 이번에도 손 안 댔다.

### 겹친 패 툴팁 "안 없어짐" — 라이브 재현 실패

같은 신고에 함께 언급된 "툴팁 안없어짐" 문제는, 신고 시점의 실제 라이브
게임 상태(9월+조커가 쌓여 있던 바로 그 필드 슬롯)를 대상으로 직접
`OnPointerDown`을 실행해 툴팁을 띄운 뒤 1초 대기했을 때 `Update()`
안전망이 정상적으로 닫는 것을 확인했다 — 이 시점엔 재현이 안 됐다.
`GoStopStackTooltip` 인스턴스도 정확히 1개, 중복 트리거도 없었다.
전 세션에 추가한 안전망(눌린 게 없으면 매 프레임 강제로 Hide) 자체는
정상 작동 중으로 보인다 — 사용자가 실제로 겪는 재현 조건이 이 확인
방식으로는 아직 못 잡힌 상태라, 다음에 재현되면 정확한 상황(터치인지
마우스인지, 어느 시점에 멈추는지)을 더 구체적으로 받아 다시 조사할
필요가 있다.

## 고스톱 — "조커가 아직도 빈 공간에 슬램다운" — 진짜 원인은 애니메이션이
슬롯 배정보다 먼저 실행되는 순서 문제 (2026-09-06)

바로 전 세션에 고친 건 **데이터(최종 렌더링) 레이어의 "죽은 무더기"
버그**였을 뿐, "빈 공간으로 슬램다운되는 것처럼 보인다"는 **애니메이션
레이어의 별개 버그**는 그때 전혀 안 건드렸다(직전 세션 문서에도 "라이브
재현 실패"가 아니라 "이건 안 고쳤다"로 명확히 적어뒀어야 했는데 사용자
응답 요약에서 빠뜨렸다). 사용자가 "고친거맞아?"로 재확인 + "조커패
2개가 있는데 하나만 수정하고 그런거아냐?"로 정확한 후보 원인까지
짚어줘서 다시 조사했다.

**원인.** `PlaySeq`의 "② 뒷패 슬램다운"(고스트 애니메이션이 실제로
어디로 떨어질지 정하는 지점)이 "③ 뻑 판정"(조커를 anchor 슬롯에
파킹시키는 `fieldSlotAssign` 배정이 일어나는 지점)보다 **코드 순서상
먼저 실행된다.** 즉 애니메이션이 목표 슬롯을 결정하는 시점엔 아직
파킹 배정이 안 돼 있어서, `FieldSlotTransform(drawn)`(drawn=조커)이
조커 전용 규칙(월이 없어 무조건 새 빈 슬롯)을 그대로 따라 엉뚱한 빈
자리로 떨어졌다 — ③이 나중에 데이터를 바로잡아 최종 렌더링은 정확한
자리에 나타나지만, **그 사이의 착지 애니메이션 자체는 계속 틀린
자리를 보여줬다.** 이건 조커가 몇 종류든(Joker_1/Joker_2 둘 다) 완전히
동일하게 겪는 문제였다 — "하나만 고쳤다"는 사용자의 가설은 합리적인
의심이었지만, 실제 원인은 카드별 분기가 아니라 **순서(②가 ③보다
먼저)** 문제였다.

**고침.** "②"의 조커 분기 맨 앞에서, ③이 나중에 쓸 것과 **완전히 같은
조건**(couldBePpeok이고, 그 달 카드가 이미 다른 좌석에 2장 이상
캡처되지 않았다면)을 미리 계산해서 `fieldSlotAssign[drawn] =
AssignFieldSlot(card)`를 여기서 먼저 실행한다 — 그러면 곧바로 이어지는
`FieldSlotTransform(drawn)`이 anchor와 정확히 같은 슬롯을 돌려준다.
③에서 같은 배정을 또 시도해도 `fieldSlotAssign` 캐시를 먼저 보므로
안전하게 멱등이라 중복 걱정이 없다.

**검증(라이브, 리플렉션 + 영구 진단 로그).** `[GoStopJokerGhost]`
로그(신규, 영구 추가 — `drawn`/`couldBePpeok`/`target`/`anchorTarget`
4가지를 고스트 생성 그 순간에 찍는다)를 붙여서, **Joker_1과 Joker_2
둘 다 각각 독립적으로 실전 시나리오(couldBePpeok+해당 조커가 다음
뒷패)를 재현** — 두 경우 모두 `target`과 `anchorTarget`이 정확히 같은
slot(`pos2`/`pos5`)으로 찍혀, 애니메이션이 시작하는 바로 그 순간부터
올바른 자리를 향하는 것을 직접 확인했다(이전엔 최종 상태만 보고
"고쳐졌다"고 판단했는데, 이번엔 고스트 생성 시점을 직접 찍어서 애니메이션
자체가 처음부터 맞는지까지 확인 — 데이터 정합성과 애니메이션 정합성은
서로 다른 검증이 필요하다는 걸 이번에 다시 확인했다). 콘솔 에러 0건.

> **교훈 — "최종 상태가 맞다"와 "그 과정(애니메이션)이 맞다"는 서로
> 다른 주장이고, 서로 다른 검증이 필요하다.** 전 세션엔 "데이터가
> 최종적으로 올바른 슬롯에 있는지"만 확인하고 "고쳤다"고 보고했는데,
> 정작 사용자가 실제로 보는 것(슬램다운 애니메이션의 착지 지점)은
...
## 고스톱 — 버그 재현 디버그 패널(`GoStop3PGame.Debug.cs`) (2026-09-07)

"나왔던 이슈들이 재발되는 케이스가 너무 많은데, 특정 상황에서만 재현되는
것들은 확인하기 힘들다 — 디버그 모드가 있어서 케이스별로 버튼이 있고
누르면 그 상황으로 세팅해주고 수정된 걸 딱 보여줄 수 있으면 좋겠다"는
요청. 나가기/도움말 버튼 오른쪽에 "디버그" 토글 버튼을 추가했다 — 누르면
15개 시나리오 버튼이 있는 스크롤 패널이 뜬다.

**설계 원칙 — 버튼은 항상 "무대만 세팅"한다, 대신 클릭해주지 않는다.**
카드를 대신 클릭하거나 팝업에 자동 응답하는 방식도 고려했지만, 실제
버그 대부분이 "클릭 이후의 흐름"(actionBusy 게이팅·애니메이션 순서·팝업
콜백)에서 나므로 사용자가 직접 카드를 눌러야 진짜 `OnPlayerPlay` 경로를
그대로 밟는다 — 리플렉션으로 몰래 우회하면 검증 가치가 떨어진다. 그래서
각 시나리오는 hand/field/drawPile/captured만 정확한 조건으로 세팅하고
`RebuildUI()`만 부른다.

**릴리즈 빌드엔 절대 안 보인다.** `Debug.isDebugBuild || Application.isEditor`
로만 토글 버튼 자체를 생성한다 — GitHub Pages에 올라가는 WebGL 배포판
(Development 빌드 체크 안 함)에는 노출되지 않는다.

**15개 시나리오** — 이 프로젝트가 반복해서 겪은 버그 카테고리를 그대로
따왔다: 폭탄, 뻑 형성, 뻑 먹기, 따닥, 쪽, 보너스패 파킹, 9월 열끗 선택,
필드 2장 선택, 흔들기, 홍단 비상(2/3), 초단 완성(3/3), 청단 실패(막힘),
3광 비상, 고/스톱 임계값, 결과 넘기기 버튼.

**월별로 격리했다.** 모든 화투 월은 카드 종류 구성과 무관하게 항상
정확히 4장이라는 사실(48장÷12월)을 이용해, 각 시나리오가 서로 다른
월(1~8·10·11월, 9월은 국열끗 전용으로 예약)을 쓰게 배정했다 — 세팅
전에 `DebugPurgeMonth(M)`으로 그 달 카드를 hand/captured/field/drawPile
전부에서 지운 뒤 `GoStopDeck.BuildFull()`에서 새로 뽑은 인스턴스를
주입한다(HwatuCard가 참조 동일성으로 다뤄진다는 이 프로젝트 기존 원칙
— 실제 딜링이 만든 카드와 테스트용 새 인스턴스가 같은 spriteName으로
화면에 중복 표시되는 걸 막는다).

**세트 진행 시나리오(비상/완성/막힘)는 클릭이 필요 없다.** `RebuildUI()`가
매번 끝에서 `CheckEmergencies()`를 호출하므로, `captured[PLAYER_SEAT]`만
정확히 채우고 `emergencyFired`/`achievedFired`/`blockedFired`를 그
세트에 한해 지운 뒤 `RebuildUI()`를 부르면 이펙트가 즉시 자동으로 뜬다.
"막힘"은 이 프로젝트의 기존 설계(비상이 이미 떴던 세트만 대상)대로
`emergencyFired`를 미리 채워 넣어야 발동한다.

**검증(Play 모드 라이브).** 토글 버튼 클릭 → 패널 15행 정상 표시 →
`HongdanEmergency` 적용 → `GoStopVectorEffect`의 alt 타이틀이 정확히
"나 홍단 비상!"으로 뜨는 것(클릭 없이 즉시) 확인. `SkipButton` 적용 →
`skipResultBtn.gameObject.activeSelf=True` 확인. `PpeokForm` 적용 →
필드/손/더미 정확히 세팅된 것 확인 → 실제로 손패 카드를 `OnPlayerPlay`로
클릭 → 필드에 3장(원래+낸 패+뒷패)이 정확히 쌓이고 캡처 0장, `ppeokCauser`
정상 기록까지 실제 클릭 경로로 확인. 이 전체 테스트 세션 콘솔 `error`/
`exception` 0건(파이프라인 자체 경고만).

> **함정 — 검증 도중 Play 모드가 원인 불명으로 조용히 중지된 적이
> 있었다.** `skipResultBtn`을 포함해 `BuildStaticUI()`가 채우는 모든
> 참조 타입 필드(`mySeatT`/`handArea`/`debugToggleBtn` 등)가 갑자기
> 전부 null로 관측돼 "심각한 버그"로 오판할 뻔했다 — `editor_status`로
> `playMode:"stopped"`를 확인하고서야 원인을 알았다: Play 모드가 멈추면
> 씬에 남은 GoStop3PGame 컴포넌트는 여전히 존재하지만(에디트 모드
> 오브젝트로), 직렬화 안 되는 private 런타임 필드는 전부 C# 기본값
> (null)으로 보인다 — `BuildStaticUI()`가 애초에 한 번도 안 돈 상태를
> 조회한 것뿐이었다. `editor_play`로 다시 들어가 재확인하니 즉시 정상
> 이었다. **"필드가 갑자기 다 null"로 보이면 먼저 `editor_status`로
> `playMode`를 확인할 것** — 이 프로젝트에 이미 기록된 "Play 모드 도중
> 재컴파일하면 도메인 리로드로 참조 필드가 리셋된다"는 함정과 증상이
> 똑같아서 처음엔 그걸로 오인했지만, 이번 원인은 재컴파일이 아니라
> Play 모드 자체의 예기치 않은 종료였다 — 재컴파일 이력이 전혀 없는데도
> 같은 증상이 날 수 있다는 뜻이라 원인 진단 시 `playMode`부터 볼 것.

**남은 것.** 나머지 9개 시나리오(따닥·쪽·보너스패 파킹·9월 열끗 선택·
필드 2장 선택·흔들기·초단 완성·청단 실패·3광 비상·고/스톱 임계값)는
로직·헬퍼 구조가 이미 검증된 3개(폭탄/뻑형성/스킵버튼/홍단비상)와
...
## 고스톱 — 나가기 팝업 전용 프리팹 분리, 필드 겹침 오프셋 방향 수정
(사용자 직접 적용) + 필드→Cap 캡처가 연속 이벤트 사이에서 순간이동하는
버그 수정 (2026-09-08)

**사용자가 직접 두 가지를 고쳐뒀다** — (1) 나가기 확인 팝업이
`ShakeConfirmPopup` 프리팹을 재사용하던 게 "오류 같아 보여서" 전용
`ExitConfirmPopup.prefab`을 새로 만들어 `BuildExitConfirmUI`가 그걸
쓰도록 코드도 같이 고쳤다(`ModalTwoButtonPopup` 컴포넌트 재사용, dim/
messageText/primaryButton/secondaryButton 정상 와이어링 확인). (2) 필드에
같은 달 카드가 여러 장 겹쳐 쌓일 때의 X축 오프셋에서 `-1f` 반전을 뺐다
(`i * step * -1f` → `i * step`, 두 지점) — "이게 맞더라"로 확인. 둘 다
그대로 커밋에 포함했다.

**"필드에서 먹은 패가 cap에 들어갈 때 어설픈 위치에 순간이동으로 뿅
한다"는 신고 — 진짜 원인은 연속 캡처 이벤트 사이의 경합이었다.**
`FillCapZone`/`SlamToCap`(2026-09-04에 만든, 필드 크기→Cap 크기로 부드럽게
줄어들며 날아가는 고스트 시스템) 자체의 좌표 계산은 여러 차례 라이브로
재확인해도 정확했다 — 단일 캡처 이벤트는 항상 정확한 최종 위치에
안착했다. 문제는 **한 턴 안에서 같은 Cap 존이 짧은 간격으로 여러 번
다시 그려지는 경우**였다: `PlaySeq`는 손패 캡처 배치 → 피 뺏기 → 덱
캡처 배치 등 여러 단계마다 `RebuildUI()`를 부르는데 그 사이 간격이
`PLAY_STEP_DELAY`(0.35초)뿐인 반면, `SlamToCap`의 비행 시간은 거리에
따라 최대 ~0.6초까지 걸릴 수 있다 — 즉 **앞선 캡처의 고스트가 아직
날아가는 도중에 다음 RebuildUI가 같은 Cap 존을 통째로 `ClearChildren`
하고 다시 그리는 상황이 실제로 벌어질 수 있었다.**

이때 벌어지는 일: (1) 목적지였던 "진짜" 카드(`CanvasGroup.alpha=0`로
숨어서 고스트를 기다리던 것)가 파괴된다. (2) `flyFrom`류 3개 딕셔너리는
매 `RebuildUI()` 끝에서 항상 `Clear()`되므로("이번 리빌드에서 움직이는
카드"만 담는 용도), 이미 캡처된 지 오래인 그 카드는 이번 리빌드
시점엔 아무 등록이 없다 — `FillCapZone.MakeOne`이 "정적 카드"로
오판해서 애니메이션 없이 즉시 나타난다. (3) 한편 옛 고스트는 이미
파괴된 목적지를 향해 계속 날아가다 `SlamToCap` 끝의 `if (rt != null)`
가드로 조용히 사라진다. 화면엔 "엉뚱한 자리로 날아가다 사라지는
고스트"(=어설픈 위치)와 "그 옆에 뜬금없이 팝업하는 진짜 카드"(=순간이동)
가 동시에 보였다 — 사용자가 보고한 두 증상이 사실 하나의 원인이었다.

**고침 — `RebuildUI()`가 끝나도 안 지워지는 별도 추적 딕셔너리
(`capFlightGhosts: Dictionary<HwatuCard, RectTransform>`)로 "지금 이
카드를 향해 날아가는 중인 고스트"를 계속 기억한다.** `SlamToCap`이
고스트를 만들 때 등록하고, 정상 완주 시(가로채기 없이 끝까지 도달)
제거한다. `FillCapZone.MakeOne`이 카드를 다시 그릴 때 `flyFrom`엔
없지만 `capFlightGhosts`엔 아직 살아있는 고스트가 있으면 — "정적
카드"로 취급하는 대신 **그 고스트의 지금 실제 위치·크기를 그대로
이어받아 새 목적지로 다시 날아가도록** `pending`에 등록한다. 옛
고스트는 그 자리에서 즉시 파괴한다(`SlamToCap`의 기존 `if (ghost ==
null) yield break;` 가드가 다음 프레임에 스스로 조용히 멈춰주므로,
옛 코루틴을 따로 신호를 보내 멈출 필요가 없다). 결과: 카드가 항상
"지금 눈에 보이는 자리"에서 이어서 움직이지, 순간이동하지 않는다.

**검증(Play 모드 라이브, 리플렉션).** ①정상 경로(경합 없음) — 단일
캡처를 여러 월(11월/4월/6월/2월 4장 동시캡처 등)로 반복 확인, 최종
위치가 기존과 동일하게 정확한 것 + `capFlightGhosts.Count`가 완주 후
0으로 정리되는 것 확인(회귀 없음). ②경합 강제 재현 — 카드를
`captured[0]`에 직접 추가하고 먼 좌표로 `flyFrom`을 리깅해 `RebuildUI()`를
호출(1차, 비행 시작) → 그 직후(같은 동기 구간) `capFlightGhosts.Count=1`
확인 → 곧바로 `RebuildUI()`를 한 번 더 호출(2차, 경합 유발) →
`capFlightGhosts.Count`가 여전히 1(누수 없음)이면서 1차 고스트는
파괴됨(`ghost1==null`)·2차 고스트는 다른 인스턴스(`sameInstance=False`,
...
## 고스톱 — 멍박 조건에서 고도리 제외 (2026-09-08)

"멍박 조건 (GoStopRules.IsLiveMeongBakRisk): 내 획득패에 열끗(끗)이
0장이어야 하고 상대(들) 중 누구든 열끗 점수(CalcScore().yeolkkeut, 5장
이상 모은 경우) 또는 고도리 점수(godori, 고도리 3장 완성)를 1점 이상
내고 있으면 → 멍박 위험 활성화 이렇게 정리해줬는데 멍박과 고도리와는
별개니까 조건에서 고도리는 빼줘"라는 정확한 지적. 이전(2026-09-04)
"광박/피박/멍박 배지를 CalcScore 기준으로 재구현" 작업에서 멍박을
`s.yeolkkeut > 0 || s.godori > 0`(OR)로 두 항목을 같이 봤는데, 사용자
확인대로 고도리는 별개 족보라 이 조건에서 빼야 맞다.

`others.Any(o => { var s = CalcScore(o, 0); return s.yeolkkeut > 0 ||
s.godori > 0; })` → `others.Any(o => CalcScore(o, 0).yeolkkeut > 0)`로
단순화했다 — 함수 앞부분의 `mine.Count(끗==0)` 조건은 그대로 유지.

**검증(Play 모드 라이브, 리플렉션)** — 상대가 고도리만 3장 완성하고
열끗 점수는 0인 경우(`yeolkkeut=0, godori>0`) → 수정 전엔 `True`,
수정 후 정확히 `False`로 바뀌는 것 확인. 상대가 열끗 5장 이상으로 실제
`yeolkkeut>0`인 경우는 수정 전후 동일하게 `True`(회귀 없음) 확인.

## 고스톱 — 필드 카드 슬램다운 오프셋이 다른 카드와 어긋나는 버그
(`RealFieldCardCount` — StackTooltipTrigger 오버레이 오카운트) (2026-09-08)

"필드 카드패 오프셋적용이 고스트패가 슬램다운되고 나서 고스트패가
사라진 이후에 되니까 애니메이션 중엔 좀 어색하거든. 슬램다운 되자마자
오프셋을 다시 맞춰줄수있나"라는 요청 — 조사해보니 두 겹의 문제였다.

**1차 원인 — "나중에 보정"하는 구조 자체.** `SpawnGhostCard(HwatuCard,
RectTransform)`는 그동안 "지금 화면에 떠 있는 카드 수(existing)"만
기준으로 고스트의 착지 offset을 계산했는데, 이 값은 그 슬롯의 **최종**
장수(고스트 자신까지 포함한 수)와 다를 수 있다 — 예: 2장짜리 슬롯에
3번째 카드가 슬램다운하면, 고스트는 "기존 2장 기준"(`FieldStackStep(2)
=20`)으로 착지하지만, 나중에 `RebuildUI()`가 다시 그릴 땐 "최종 3장
기준"(`FieldStackStep(3)=15`)으로 재계산된다 — 그 차이를
`DrawField()`의 `flyFrom` 기반 `SlamIn`(플레인 슬라이드)이 **고스트가
슬램다운 임팩트+펀치스케일을 끝내고 사라진 다음**에야 뒤늦게 보정했다.
그래서 카드가 "쾅! 착지 → (한 박자 뒤) 슬쩍 밀림"이라는 2단 동작으로
보였다. 게다가 같은 슬롯에 이미 있던 다른 카드들(형제들)은 `flyFrom`
등록이 없어 이 보정 때 **애니메이션 없이 조용히 스냅**됐다.

**고침 — 고스트를 만들기 직전에 형제 카드들을 최종 step으로 먼저
재배치한다.** 새 헬퍼 `RelayoutFieldSlotSiblings(target, newCount)`
(GoStop3PGame.UI.cs)가 이 슬롯의 기존 실카드들(고스트·오버레이 제외)을
`newCount` 기준 최종 offset으로 즉시 옮긴다 — 이미 정확한 자리에
있으면 손 안 대고, 실제로 옮겨야 하면 기존 `SlideHandCard`(smoothstep
슬라이드, 손패 재배치에 이미 쓰던 코루틴을 그대로 재사용)로 부드럽게
이동시킨다. `SpawnGhostCard`는 이제 `finalCount = existing+1`을 먼저
계산해 `RelayoutFieldSlotSiblings`를 호출한 뒤, 고스트 자신도 처음부터
그 최종 offset으로 곧장 슬램다운한다 — "착지 = 최종 자리"라 이후
`RebuildUI()`가 다시 그려도 위치가 안 바뀌므로 2차 보정 자체가 필요
없다.

**2차 원인(라이브 실측으로 발견한 진짜 버그) — `existing`이
`target.childCount`를 그대로 썼는데, 이 값이 `StackTooltipTrigger`
(2장 이상 쌓인 슬롯마다 DrawField가 얹는 투명 터치 오버레이, 카드가
아니다)까지 "카드 한 장"으로 잘못 세고 있었다.** 실카드 2장짜리
슬롯인데 오버레이까지 포함해 `existing=3`으로 계산되는 바람에
`finalCount=4, step=FieldStackStep(4)=10`이 나와, 진짜 정답(3장 기준
`step=15`)과 다른 잘못된 간격으로 재배치됐다 — 우연히 **고스트
자신의 좌표만 계산이 겹쳐 맞아 보였을 뿐**(3*10 == 2*15 == 30이라는
숫자 우연), 실제로는 기존 카드가 틀린 자리(예: 15가 아니라 10)로
재배치되는 진짜 회귀였다. 새 정적 헬퍼 `RealFieldCardCount(target)`
(StackTooltipTrigger만 이름으로 제외하고 세는 카운터, 고스트는
"곧 이 슬롯에 자리 잡을 카드"라 그대로 포함)로 교체해서 고쳤다.

**검증(Play 모드 라이브, 리플렉션) — 두 단계로 나눠 확인했다.**

①**고립된 단위 테스트**(`SpawnGhostCard`를 직접 호출, `field`에 실제
카드 2장만 구성한 "깨끗한" 달을 골라 다른 달의 오염을 피함) — 수정
1차(RelayoutFieldSlotSiblings 도입, `existing` 버그는 아직 남은 상태)
에서는 형제 카드가 `(10,-10)`(잘못된 step=10 기준)으로 재배치되는
회귀를 직접 실측으로 잡아냈다 — 처음엔 이게 "배경에서 자연 진행되는
AI 턴과의 경합"인가 의심했지만, `field`의 해당 달 카드 수가 테스트
전후로 전혀 안 바뀐 것까지 확인해 진짜 계산 버그임을 확정했다.
`RealFieldCardCount` 적용 후 재검증 — 형제 카드가 정확히 `(15,-15)`
(3장 기준 step=15와 정확히 일치)로 부드럽게 슬라이드해 정착하는 것,
고스트도 `(30,-30)`(i=2,step=15)으로 처음부터 정확히 착지하는 것까지
확인.

②**실제 프로덕션 플레이 경로**(디버그 패널 `JokerPark` 시나리오로
5월 무매칭 손패를 실제 `OnPlayerPlay`로 재생 — 조커 파킹 후 다음 뒷패가
같은 달이라 뻑이 형성되는 케이스) — 최종 필드 상태가
`[May_Tane@(0,0), Joker_1@(15,-15), May_Tanzaku@(30,-30)]`로 3장 전부
`FieldStackStep(3)=15` 기준 정확한 계단식 간격에 안착(겹침·어긋남
없음), `ppeokBonusPi[5]=Joker_1` 정상 기록까지 확인 — 기존에 검증된
뻑 형성 로직 자체는 이번 변경(순수 렌더링 레이어)과 무관하게 그대로
살아있음을 재확인했다.

이 세션 전체(고립 테스트 + 프로덕션 플레이 경로) 콘솔 `error`/
`exception` **0건**.

> **교훈 — "우연히 숫자가 맞아 보이는" 결과를 최종 확인으로 삼지
> 말 것.** 1차 수정 직후 고스트의 착지 좌표만 확인했을 땐 정확히
> 의도한 값(30,-30)이 나와서 "고쳐졌다"고 판단할 뻔했다 — 실제로는
> `existing`(잘못 계산된 3) × `step`(잘못 계산된 10) = 30이
> `finalCount-1`(정확한 2) × `step`(정확한 15) = 30과 **수치적으로
> 우연히 일치**했을 뿐이었다. 형제 카드(간접적으로만 영향받는 값)까지
> 같이 확인하고 나서야 잘못된 계산 경로였다는 게 드러났다 — 여러 값이
> 같은 공식을 공유할 땐, 그 공식에 의존하는 값 중 **하나만이 아니라
> 여럿**을 교차 확인해야 우연한 일치에 속지 않는다.
...
## 고스톱 — CPU 캐릭터 로스터 22명 확장 (2026-09-08)

"인터넷 서칭해서 고스톱, 화투 관련한 캐릭터 10명~30명만 더 리스트업 좀
해줘. 리스트업 되면 지금 캐릭터에 능력치 부여한것처럼 캐릭터에 맞게
티어와 스킬 부여해줘." 기존 13명이 전부 영화 "타짜"(1부/원작 만화
1부)였으므로, 웹 검색(나무위키)으로 **같은 세계관의 다른 파트**
(만화 2부·3부, 영화 "신의 손"·"원 아이드 잭")에서 22명을 새로 골라
`GoStopCharacters.All`에 추가했다 — 기존 13명과 겹치지 않게 확인
(중복 이름 0개, 총 35명).

**출처**: 원 아이드 잭(도일출·애꾸·영미·타짜마귀), 신의 손(대길·꼬장·
송마담·서실장), 원작 만화 2부(허미나·허광철·장동식·안인길·박영희·
우지연), 원작 만화 3부(나라·마돈나·이현지·변태섭·제갈공배·최광수·
최동수·허전). 이미 검증된 원칙("이름·성격만 참고, 대사·작화는 안
베낀다")을 그대로 지켰다 — 나무위키 인물 소개 문서에서 성격만 추출.

**티어·스킬 배정 근거는 코드 주석에 22명 전원분을 캐릭터별로 남겼다**
(`GoStopCharacters.cs`, `All` 배열 직전 주석 블록) — 예: 타짜마귀
(원 아이드 잭 최종보스, 20년간 판을 설계한 흑막)는 `cardCountingSkill`·
`allyTargetingSkill` 최상급에 `goAggression`은 낮게(무리하게 안 걺),
최동수(정보수집 담당 "모니터 요원")는 그 정체성 자체가
`cardCountingSkill`에 대응. 티어 분포는 13A/9B/13C(전체 35명 기준)로,
기존 13명(4A/4B/5C)과 합쳐도 극단(A=마스터급/C=호구급) 위주로 쏠리는
드라마 캐릭터 특성상 자연스러운 분포다.

**검증(Play 모드 라이브, 리플렉션).** `GoStopCharacters.All.Length=35`,
`GroupBy(tier)` 집계가 정확히 13A/9B/13C인 것 확인. 개별 캐릭터
(허미나·최동수)의 `skills` 필드가 코드에 적은 값과 정확히 일치하고,
override 안 한 축은 티어 기본값 그대로인 것(예: 허미나 tier A의
`BackingChance`가 명시 안 했으므로 A 기본값 `1f`) 확인. `DrawRandom(3)`
호출이 예외 없이 3명을 뽑는 것 확인. **실제 4인 게임을 새로 시작해서**
좌석에 새 캐릭터(타짜마귀·점박이·장동식)가 정확한 티어·시드머니
(A=100만/C=10만)로 배정되는 것까지 end-to-end로 확인 — 컴파일 클린,
콘솔 `error`/`exception` 0건.

## 최적화 — 1~50 게임 타이머 표시 갱신을 프레임당→값 변경 시로 throttle
(2026-09-08)

"이제 최적화 작업을 할거야. 간단하거나 현재 시스템에 문제없을것
리스팅하고 적용해줘" 요청 — GC 할당/성능 관점에서 프로젝트 전체 `Update()`
루프(`grep -rl "void Update()" Assets/Scripts/` 기준 14개 파일)를
전수 조사했다.

**대부분 이미 잘 최적화돼 있었다** — `Camera.main`은 두 곳 다 필드에
캐싱해서 재사용 중(`BrickBreakerFX`/`BrickBreakerManager`), `SafeArea`/
`GoStopNetLobbyUI`/`GoStopStackTooltip`은 전부 조기 반환 가드나 명시적
스로틀(0.5초 간격 등)이 이미 있었다. `BrickBreakerBrick.UpdateVisual()`
(텍스트 갱신 포함)은 이벤트 시점(피격·스폰·타겟팅 등)에만 불리고
`Update()` 자체엔 spin 회전 외 다른 로직이 없음을 확인했다 — 이미
검증된 시스템(BrickBreaker3D의 정교한 오디오·깊이감 렌더링)이라 손대지
않았다.

**유일하게 찾은 실제 낭비 — `Game1to50.Update()`.** `ui?.SetScore(
$"{elapsed:F1}s")`가 게임이 진행 중인 동안 **매 프레임**(60fps 기준
초당 60회) 새 문자열을 만들어 TMP 텍스트를 갱신하고 있었다 — 그런데
화면엔 소수 첫째자리(F1)까지만 보이므로 실제로 표시가 바뀌는 건 초당
10번뿐이다. 나머지 50번은 전부 같은 문자열을 다시 만들어 다시 대입하는
낭비였다.

**고침** — `lastShownTenths`(int) 필드로 "지금까지 화면에 보여준
0.1초 단위 값"을 기억해 두고, `(int)(elapsed*10f)`가 그 값과 실제로
달라졌을 때만 문자열을 새로 만들어 `SetScore`를 부른다. `elapsed` 누적
자체나 `ShowResult()`/`OnTap()`이 읽는 원본 `elapsed` 값은 전혀 안
건드렸다 — 순수하게 "화면 갱신 빈도"만 실제 표시 정밀도에 맞췄다.
`StartGame()`에서 `lastShownTenths = -1`로 같이 리셋해야 했다 — 안
하면 재시작 직후 이전 판의 스테일한 값과 우연히 같은 구간 동안 화면이
"0.0s"에 얼어붙어 보이는 회귀가 생긴다(실측으로 이 경우를 미리 예상하고
리셋을 같이 추가).

**검증(Play 모드 라이브, 리플렉션).** 초기 진행 중 `scoreText`가
`elapsed`의 F1 반올림과 정확히 일치하는 것(8.5s→16.3s로 1.2초 뒤 정상
갱신 확인), 재시작 직후 `lastShownTenths=-1, elapsed=0`으로 정확히
리셋되는 것 + 재시작 후 다시 정상 갱신되는 것(0.5초 뒤 5.2s로 확인),
50개 전부 실제 `OnTap()`으로 클릭해 게임을 완주시켜 `overlayScore`
(F2, "12.86초")·`bestText`(F1, "최고 12.9s")가 원본 `elapsed` 값과
정확히 일치하는 것까지 확인 — 승리·최고기록 경로는 전혀 회귀 없음.
콘솔 `error`/`exception` 0건.

## 고스톱 — 보너스패가 필드 패 위에 쌓일 때 위치가 완전히 어긋나는 버그
(`RealFieldCardCount`가 "Impact" 임팩트 플래시를 카드로 오인) (2026-09-08)

"보너스패 필드에 쌓일 때 포지션 확인해봤어?" → "정확하게는 필드 패 위에
쌓일때 포지션이 완전이상함" — 바로 전 세션에 고친 `RealFieldCardCount`
(StackTooltipTrigger 오버레이 오카운트 수정)가 놓친 **두 번째 오버레이
오카운트 버그**였다.

**원인.** `SpawnImpactFlash(RectTransform at)`(카드가 착지할 때 뜨는 흰
원 임팩트 플래시)가 `go.transform.SetParent(at.parent, false);`로 —
즉 방금 착지한 카드(`at`)와 **같은 부모**(필드 슬롯 자체)에 형제로
붙는다. `RealFieldCardCount`/`RelayoutFieldSlotSiblings`는 그동안
"StackTooltipTrigger"라는 이름만 걸러내고 "Impact"는 몰랐다 — 그래서
카드가 착지한 직후, `Impact`가 아직 자기소멸(`FlashAndDestroy`, 짧은
페이드 후 스스로 파괴)하기 전인 그 짧은 창 안에 **다음 카드**(이번
경우 조커)가 같은 슬롯에 슬램다운하면, `SpawnGhostCard`가 계산하는
`existing`(그 슬롯에 이미 있는 "진짜 카드" 수)이 실카드 1장을 2장으로
잘못 셌다 — `finalCount=3, step=FieldStackStep(3)=15`가 되어 진짜
정답(`finalCount=2, step=20`)과 다른 촘촘한 간격으로 착지 offset이
계산됐다(`existing(2)*15=30` vs 정답 `existing(1)*20=20`). 라이브
재현으로 실제 관측한 값(X=30, 심지어 hover 단계에서는 -836/-216
같은 훨씬 큰 오프셋도 관측 — 이건 조커가 아직 덱더미 위에서 뒤집기
연출 중인 정상 단계였고, 진짜 문제는 그 다음 "착지" 단계의 X=30
오프셋이었다)이 정확히 이 계산식(existing=2 오판)과 일치했다.

**고침.** `IsFieldSlotOverlay(string name)` 공용 헬퍼를 새로 만들어
`"StackTooltipTrigger"`와 `"Impact"` **둘 다** 걸러내도록 통일하고,
`RealFieldCardCount`/`RelayoutFieldSlotSiblings` 양쪽 다 이 헬퍼로
교체했다 — 앞으로 세 번째 오버레이 종류가 생겨도 한 곳만 고치면
된다.

**검증(Play 모드 라이브).** ①**고립 단위 테스트** — 필드에 카드 1장만
있는 깨끗한 슬롯에 `SpawnImpactFlash`와 똑같은 방식(`SetParent(target,
false)`, 이름 "Impact")으로 가짜 오버레이를 직접 끼워 넣은 뒤
`SpawnGhostCard`를 호출 — 새 카드가 정확히 `(20,-20)`(existing=1로
올바르게 계산, Impact 제외)에 착지하는 것을 확인했다(수정 전이었다면
`(30,-30)`이 나왔을 상황 — 실제로 라이브에서 관측했던 그 잘못된 값과
정확히 일치). ②**실제 프로덕션 재현** — 필드에 이미 카드 1장이 있는
달의 손패를 실제로 `OnPlayerPlay`로 재생하고 다음 뒷패를 조커로 리깅
(`Time.timeScale`을 0.08배로 늦춰서 배경 자연 진행과의 경합 없이
착지 순간을 정밀하게 캡처) — 조커가 착지 직전엔 `(20.00, 30.71)`
(아직 낙하 중, X는 이미 올바른 20으로 확정, Y만 위에서 떨어지는 중),
착지 완료 시 `(20.00, -20.00)`(Impact 플래시가 같은 자리에 잠깐 같이
뜬 뒤 소멸)로 정확히 안착하는 것을 확인했다 — 신고된 "완전 이상함"이
실제로 사라졌다. 콘솔 `error`/`exception` 0건.

> **검증 방법론 — `Time.timeScale`을 낮춰 애니메이션을 늘려 잡는 기법을
> 처음 이렇게 정밀하게 활용했다.** 이 프로젝트가 이미 여러 번 겪은
> "리플렉션 폴링이 배경 자연 진행(AI 턴)과 경합해 중간 상태를 못 잡는다"
> 는 함정을, 실시간 sleep 간격을 줄이는 대신 **게임 자체의 시간을
> 늘려서** 우회했다 — `Time.timeScale=0.01~0.08` 구간에서 몇 초의
> 리플렉션 왕복 지연 동안에도 애니메이션이 실제로는 아주 조금만
> 진행되므로, 정확한 중간 프레임(착지 직전/직후)을 안정적으로 캡처할
> 수 있었다. 너무 낮추면(0.01) 발동까지 너무 오래 걸리고, 적당히
...
## 고스톱 — 필드 카드 홀수 개수는 정상 현상, 100턴 실측으로 카드
보존(총 50장) 확인 (2026-09-08)

"필드에 패가 홀수개가 되는 케이스는?"이라는 질문에 이론만으로 답하지
않고 실측했다 — 4인 게임을 리플렉션으로 자동 진행(참가선언·딜러뽑기·
흔들기·9월열끗·필드선택·고/스톱 팝업을 전부 자동 응답, GameOver 시
재시작 포함)시키며 매 폴마다 `hand+field+captured+drawPile` 총합을
검사했다.

**답 — 필드 카드 수가 홀수인 것 자체는 100% 정상이고 버그 신호가
아니다.** 손패든 뒷패든 상대 카드와 안 맞아 필드에 그냥 놓이는 매
순간마다 필드 총량이 ±1씩 바뀐다(짝↔홀 반복) — 이건 게임 진행 중
끊임없이 일어나는 자연스러운 변화다. 실측으로도 100회 폴 중 필드가
홀수였던 경우가 다수(짝/홀이 섞여서 계속 나타남) 확인됐다.

**진짜로 확인해야 하는 것은 "홀짝"이 아니라 "카드가 실제로 사라졌는가"다.**
이 게임은 항상 48장(표준) + 조커 2장 = **정확히 50장**이 hand+field+
captured+drawPile 네 곳 중 어딘가에 있어야 한다(2026-08-19/20에 있었던
"조커가 필드에 미아로 남는다" 버그가 바로 이 불변식이 깨진 사례였다).

**측정 방법 — 첫 시도에서 함정에 걸렸다가 고쳐서 재측정.** 처음엔
`actionBusy`(플레이어 클릭 처리 중 가드) 플래그로 "지금 카드가 공중에
날아다니는 중이 아닌지"를 판별하려 했는데, 이 플래그는 **플레이어
자신의 클릭에만 세팅되고 AI 턴에는 전혀 세팅되지 않는다**(코드 확인 —
`actionBusy = true`는 `OnPlayerPlay`/`OnPlayerBombSkip` 안에만 있음) —
그래서 AI 턴이 카드를 옮기는 도중(`PlaySeq`의 `WaitForSeconds` 단계
사이)에도 `actionBusy=False`로 읽혀서, 정말로 카드가 손/필드/캡 어디에도
"아직 없는"(고스트로만 날아다니는 중인) 순간을 "정착 상태인데 total≠50"
이라는 가짜 버그로 잘못 잡을 뻔했다(50회 중 15회가 이렇게 오탐됐다).
**고침** — `capFlightGhosts`(필드→Cap 비행 추적 딕셔너리)와 `GhostMarker`
컴포넌트(필드 슬램다운 비행 표시, `FindObjectsByType`로 씬 전체에서
카운트)가 **둘 다 0개**일 때만 "진짜로 아무 카드도 안 날고 있다"로
판정하도록 재설계했다.

**결과 — 100회 폴 중 진짜 정착 상태로 확인된 32회 전부 total=50.**
비행 중이라 판정을 건너뛴 나머지 68회를 빼면, **판정 대상 전부가 예외
없이 정확히 50장**을 유지했다 — 카드가 사라지거나 중복되는 사례는
단 한 건도 없었다. 도중에 자연 진행으로 재딜(NewGame)이 한 번 일어나
조커 2장이 새로 섞여 들어갔고, 최종 상태를 확인해보니 두 조커 모두
`captured[3]`(한 좌석)에 깔끔하게 들어가 있었다(미아 없음, 중복 없음).
콘솔은 이 100턴 스트레스 테스트 세션 전체에서 `error`/`exception`
0건.

**결론.** 필드 카드 개수의 홀짝 자체는 신경 쓸 필요가 없다. 이 세션의
여러 위치-버그 수정(Impact 오버레이 오카운트, `handActualLanding` 피벗
오류, `fieldArea.position` 오용)은 전부 **순수 시각적/애니메이션
좌표 계산**만 건드렸을 뿐 `field.Add`/`field.Remove`/`captured.Add` 같은
실제 카드 소유권 이동 로직은 손대지 않았으므로, 카드 보존 불변식이
깨질 새 위험을 만들지 않았다는 것도 이번 실측으로 함께 확인됐다.

> **교훈 — "정착 상태인지"를 판별하는 플래그를 고를 때, 그 플래그가
> 정말로 모든 좌석·모든 코루틴에 공통으로 적용되는지 먼저 확인할
> 것.** `actionBusy`는 이름만 보면 범용 "지금 뭔가 진행 중" 플래그처럼
> 보이지만 실제로는 "플레이어 클릭 가드"라는 좁은 용도였다 — 코드를
> 안 읽고 이름만 믿었다면 존재하지 않는 버그를 "발견"해 보고할 뻔했다.

## 고스톱 — 필드 홀짝 정정: "폭탄+뒷패 한 장"은 사실 홀짝을 안 바꾼다,
진짜 원인은 "이번 턴 이벤트가 1개뿐인가 2개인가" (2026-09-08)

바로 위 섹션에서 "필드 홀짝은 아무 때나 자연스럽게 바뀌는 정상 현상"
이라고 답했는데, 사용자가 정확한 반례를 제시했다: "각턴이 끝나고
필드의 총량이 홀수로 바뀌는건 굉장히 특이 케이스로 폭탄을 하고
더미패를 한장 깐 이후 밖에 없는 걸로아는데." 실측(라이브 AI 자동
진행 폴링)만으로는 안 걸릴 만큼 미묘한 패턴이라, 이번엔 라이브 게임과
경합하지 않는 **결정적(deterministic) 순수 로직 테스트**로 정확한
메커니즘을 다시 검증했다.

**핵심 — `GoStopRules`의 모든 개별 카드 판정(Resolve 계열) 함수는
`field.Count`를 항상 홀수 크기(odd magnitude)로만 바꾼다.** 소스를
직접 추적해 확인:
- `Resolve`(무매칭): `field.Add(played)` → **+1**
- `Resolve`(1:1 매칭) / `ResolveChoice`(2장 중 선택): `field.Remove(1장)` → **-1**
- `Resolve`(3장 뻑 먹기) / `ResolveJokerPpeok`(2장+조커 스윕): **-3**
- `ResolveWithBomb`(폭탄, 필드 카드 정확히 1장 제거): **-1**

**정상 턴은 이 개별 이벤트가 정확히 2번** — 손패 1장 + 뒷패 1장(둘 다
`willDraw = drawPile.Count > 0`이면 항상 일어난다, 폭탄 여부와 무관 —
2026-09-07에 이미 "폭탄도 덱을 정상적으로 넘긴다"로 확정한 규칙).
**홀수+홀수=항상 짝수**이므로, 정상 턴(폭탄 턴 포함)은 필드 총량을
항상 짝수만큼만 바꾼다 — **즉 폭탄+뒷패 한 장은 홀짝을 안 바꾼다.**

**결정적 검증(라이브 게임과 완전히 분리된 합성 데이터, 타이밍 경합
없음):**
```
wasBomb=True r1.matchCount=1 r1.captured.Count=4
fieldBefore=3 fieldAfterBomb=2(Δ-1) fieldAfterDeck=3(Δ+1)
전체턴델타=0 홀짝flip=False
```
필드 3장(홀) → 폭탄으로 2장(짝, 여기서 폭탄 단독으로는 실제로 홀짝이
바뀐다) → 뒷패 무매칭으로 다시 3장(홀) — **완결된 한 턴 기준으로는
원래 홀수 그대로**, 부호가 정확히 상쇄됐다.

**진짜 홀짝이 바뀌는 조건 — "이번 턴 이벤트가 2개가 아니라 1개뿐일
때"만.** 이것도 같은 방식으로 결정적 검증:
```
A(덱만넘김, 손패 없음): 2->3 홀짝flip=True
B(덱없어손패만, 덱 바닥남): 2->3 홀짝flip=True
```
- **A**: 손패가 0장이라 `DeckOnlySeq`가 도는 턴 — 뒷패 판정 1번뿐.
- **B**: `drawPile.Count==0`이라 `willDraw=false`가 되는 턴 — 손패
  판정 1번뿐(뒷패 없음).

둘 다 라운드 후반(손패/덱이 바닥나는 시점)에 흔히 일어나는, **폭탄과
무관한** 일반적인 상황이다. 이 두 경우만 정확히 이벤트 1개(홀수 델타)
라서 필드 홀짝이 실제로 뒤집힌다.

**사용자의 "폭탄" 연상이 아주 틀린 건 아니었던 이유.** 폭탄 자체는
홀짝을 안 바꾸지만, **폭탄은 손패 3장을 한 번에 써서 그 좌석의 손이
훨씬 빨리 바닥난다** — 그래서 "손패 0장 → 케이스 A(덱만넘김)"가 그
좌석에게 정상 플레이보다 훨씬 일찍 찾아온다. 실제 홀짝 반전은 폭탄
치는 그 턴이 아니라 **그 뒤에 이어지는, 손이 빈 채로 덱만 넘기는
턴**에서 일어난다 — 폭탄이 그 계기를 앞당길 뿐, 폭탄 없이도 라운드
막바지엔 어차피 모두가 겪는 현상이다.

**정정.** 바로 위 섹션의 "필드 홀짝은 아무 때나 자연스럽게 바뀐다"는
답은 결론(카드 손실 버그는 아니다)은 맞았지만 **메커니즘 설명이
부정확했다** — 실제로는 "정상 2-이벤트 턴은 홀짝을 보존하고, 손/덱이
바닥나는 라운드 후반의 1-이벤트 턴에서만 홀짝이 뒤집힌다"는 훨씬
좁고 정확한 규칙이었다. 100턴 라이브 스트레스 테스트에서 관측된 잦은
홀짝 변화(36/60)는 실제로는 그 테스트가 자연 진행되며 여러 좌석의
손패가 여러 번 바닥났다 다시 딜링되는 과정을 거쳤기 때문일 가능성이
높다 — 재검토는 안 했지만 이 정정된 규칙과 모순되지 않는다.

## 고스톱 — 필드 홀짝 3차 정정: 진짜 원인은 폭탄이 아니라 조커
(2026-09-08)

바로 위 정정("정상 턴은 홀짝을 보존, 손/덱이 바닥나는 1-이벤트 턴에서만
뒤집힌다")에 사용자가 반례를 줬다: "아무도 폭탄을 하지않았는데 필드에
홀수개 패가 있던적이있어서." 다시 소스를 추적해서 **폭탄과 완전히
무관한, 세 번째 메커니즘**을 찾았다 — 바로 **조커**다.

**원인 — 조커는 "매달 매칭"에 참여하지 않아서 홀수 크기 규칙을 깬다.**
정상 카드는 `GoStopRules.Resolve` 계열을 거쳐 항상 홀수 크기(±1/±3)로
field를 바꾼다는 게 지금까지의 전제였는데, **조커는 이 함수들을 아예
안 거친다** — `GoStop3PGame.CaptureBonusJokerImmediately`가 대신
처리하는데, 이 함수는:
```csharp
if (!field.Contains(joker))
{
    field.Add(joker);       // +1
    ...
    field.Remove(joker);    // -1
    cap.Add(joker);
}
```
**"손패 쪽이 이미 뭔가 캡처한 상태(폭탄/2장선택/뻑먹기 스윕 — 셋 다
`r1.captured.Count > 0`)에서 뒷패가 조커면"**, `PlaySeq`가
`anchor = r1.captured.Count == 0 ? card : null;`로 `anchor=null`을
넘기고, `ResolveBonusJoker`는 `anchor==null`이면 바로 이 함수로
직행한다 — **조커가 field에 추가됐다가 곧바로 다시 제거되면서 순
델타가 정확히 0(짝수)이 된다** — 카드 하나가 "이벤트"를 소비했는데도
홀수가 아니라 짝수 기여를 한 셈이다. 손패 쪽 델타는 여전히 홀수(매칭
-1, 뻑먹기 -3, 폭탄 -1)이므로: **홀수(손패) + 짝수(조커) = 홀수 총합
→ 홀짝 뒤집힘.** 폭탄은 `r1.captured.Count>0`을 만드는 **세 가지
경로 중 하나일 뿐**이라 — 2장 선택 매칭이나 3장 뻑먹기 스윕만으로도
똑같이 이 조건을 만족한다.

**결정적 검증(폭탄 완전히 배제, 순수 로직):**
```
wasBomb=False r1.matchCount=3 r1.captured.Count=4
field: 4(시작) -> 1(뻑먹기 후,Δ-3) -> 2(조커 파킹,Δ+1) -> 1(조커 즉시캡처,Δ-1)
이번 턴 전체 델타=-3 홀짝flip=True (폭탄 전혀 없음)
```
필드가 4장(짝)으로 시작해서, **뻑먹기(3장 스윕, 폭탄 아님)** 로 -3
(1장, 홀) → 조커가 잠깐 나타났다(2장) 곧바로 다시 사라지며(1장) 순
델타 0 → 최종 1장(홀), 시작 대비 홀짝이 뒤집혔다. 손패로 2장 선택
매칭을 완성한 뒤 조커가 뒷패로 나오는 경우도 수식상 완전히 동일한
메커니즘이다.

**추가로 발견한 관련 케이스 — 조커가 "낀 채로" 뻑이 형성되는 경우.**
손패가 무매칭으로 필드에 놓인 뒤(couldBePpeok) 뒷패가 조커라 그 자리에
파킹되고, **그 다음 카드가 실제로 같은 달**이면 뻑이 형성되는데 —
이때 무더기에 물리적으로 **4장**(원래 필드 카드+손패+조커+마지막
카드)이 쌓인다(보통의 뻑 형성은 3장인데, 조커가 하나 더 낀 것). 이
경우도 매칭(+1 무매칭)+파킹(+1)+매몰(+1) = **+3(홀수) → 홀짝 뒤집힘**,
역시 폭탄과 무관하다. 반대로 그 다음 카드가 다른 달이면("뻑 아님")
조커는 파킹됐다가 나중에 제거되고(-1) 그 카드는 독립적으로 정상
판정되므로 — 이 경우는 다시 짝수로 상쇄돼 홀짝이 보존된다(조커가
"낀 채로 뻑이 완성될 때"만 홀짝이 깨진다).

**최종 정리 — 필드 홀짝을 뒤집는 조건 4가지(전부 정상 동작, 버그
아님):**
1. 손패가 없어 덱만 넘기는 턴(라운드 후반, 흔함).
2. 덱이 없어 손패만 내는 턴(라운드 막바지, 흔함).
3. 손패가 뭔가를 캡처(매칭/선택/뻑먹기/**폭탄**) 했는데 그 직후 뒷패가
   조커인 경우 — **폭탄은 이 셋 중 하나일 뿐, 폭탄이 아니어도 똑같이
   일어난다.**
4. 무매칭 손패 위에 조커가 파킹된 뒤, 진짜 카드가 나와 조커까지 낀
   4장짜리 뻑이 형성되는 경우.

바로 위 섹션의 "정상 2-이벤트 턴은 항상 짝수 델타"라는 규칙은 **조커가
관여하지 않는 카드에 한해서만** 정확하다 — 조커가 끼면 그 자체가
"홀수 크기 규칙"을 깨는 예외라서 별도로 다뤄야 했다. 사용자의 재반례
덕분에 이 예외를 놓치지 않고 잡을 수 있었다.

## 고스톱 — 필드 홀짝 4차 정정: "카드 총량"과 "스택 가짓수"는 서로 다른
지표다 (2026-09-08)

바로 위 두 정리(폭탄/뻑먹기/2장선택+조커, 뻑형성+조커)를 사용자가
재확인하면서 결정적인 구분을 지적했다: **사용자가 원래 물어본 건
`field.Count`(카드 총량)가 아니라 "스택의 가짓수"(필드에 몇 개의 서로
다른 달 무더기가 있는가)였다.** "두 번째(뻑 형성+조커)는 맞는 얘기고,
카드 총량은 홀수가 되지만 스택 가짓수는 항상 짝수 — 내가 원래
의문이었던 건 스택 가짓수가 홀수였던 거다."

**결정적 검증 — 카드 총량과 스택 가짓수를 나란히 추적.**
```
[뻑먹기(3장 스윕, 폭탄 아님)+조커] 카드 4→1→1(flip=True) | 스택 2→1→1(flip=True)
[2장선택 매칭(뻑먹기 아님)+조커]   카드 3→2→2(flip=True) | 스택 2→2→2(flip=False)
```

**두 지표가 갈리는 이유.** "이전 섹션의 케이스 1"(손패가 뭔가 캡처한
직후 뒷패가 조커)은 캡처의 **종류**에 따라 스택 가짓수 영향이 다르다:
- **폭탄 / 뻑먹기(3장 스윕)** — 그 달의 카드 전부(1장 또는 3장)를
  가져가므로 그 달의 **스택 자체가 필드에서 완전히 사라진다**(스택
  -1). 여기에 조커의 순 기여(0, 파킹→즉시제거)가 더해져도 **스택
  가짓수가 홀수로 뒤집힌다.**
- **2장 선택 매칭** — 필드에 원래 2장 있던 그 달 카드 중 1장만
  가져가고 1장은 그대로 남는다 — **스택 자체는 안 사라지고 그냥
  1장으로 줄어들 뿐**(스택 변화 0). 조커의 기여도 0이므로 **스택
  가짓수는 안 바뀐다** — 카드 총량만 홀수가 되고 스택 가짓수는 짝수
  그대로다.

**결론 — "케이스 1"을 스택 가짓수 기준으로 정확히 다시 쓰면:** "손패
쪽 캡처가 **폭탄이거나 뻑먹기(3장 스윕)** — 즉 그 달의 스택을 통째로
비우는 캡처였고, 그 직후 뒷패가 조커면" 스택 가짓수가 홀수로
뒤집힌다. 2장 선택 매칭은 이 조건에서 빠진다. "폭탄 안 했는데 스택이
홀수였다"는 관측은 정확히 **뻑먹기+조커** 조합이었을 가능성이 가장
높다(지난 섹션의 결정적 재현 예시도 바로 이 조합이었다 — 우연히
스택 가짓수 검증까지 겸하고 있었던 셈).

**"케이스 2"(뻑 형성+조커)는 사용자 말대로 정확하다** — 원래 있던
1장짜리 스택이 (원래 필드 카드+손패+조커+마지막 카드) 4장으로
불어날 뿐, 새 스택이 생기거나 기존 스택이 사라지지 않으므로 **스택
가짓수는 항상 짝수로 유지**된다 — 카드 총량만 홀수가 된다.

**교훈 — "필드 총량"이라는 말이 두 가지로 해석될 수 있었다.** 이번
전체 조사가 여러 차례 정정을 거친 이유가 여기 있었다 — "필드에 패가
홀수개"라는 최초 질문을 `field.Count`(카드 실물 개수)로만 해석하고
"스택 가짓수"(육안으로 셀 때 무더기가 몇 개인지)라는 별도 지표는
전혀 고려 안 했다. 두 지표는 대부분의 이벤트에서 같은 방향으로
움직이지만(무매칭 카드 등), **매칭·선택·스윕처럼 "필드 카드가
...
## 고스톱 — "마지막턴" 재정의: 손패 소진이 아니라 개인 턴 번호(7/10)
+ 보너스패 다음 뒷패 로그 누락 (2026-09-08)

바로 위 "뻑/자뻑도 마지막턴엔 피뺏기 억제" 직후 사용자가 정확한 반례를
들며 정정 — "쪽, 쓸, 따닥, 뻑먹기 발동안하는 마지막턴을 마지막 손패로
한다는게 더미카드는 카운팅안하는거니? 유저의 마지막턴은 일반고스톱에서는
유저의 7번째턴, 맞고에서는 10번째 턴이 되어야되는데... 폭탄을 하더라도
마지막턴이 변경되면 안되." 예시로 준 "손패3+더미2→3번째턴", "손패1+더미2→
5번째턴"을 대입해보니, **더미(drawPile) 장수는 실제로 이 판정에 전혀
안 들어가고, "originalHand-currentHand+1" 공식(딜링 손패를 5장으로 두면
정확히 3/5 일치)만으로 두 예시가 완벽히 재현됐다** — 즉 진짜 요구사항은
"손패가 빈 순간"이 아니라 **"이 좌석의 실제 개인 턴 횟수가 정확히
HandSizeLimit(고스톱 7/맞고 10)번째에 도달했는가"**였다.

**왜 이게 예전 h.Count==0 판정과 다른가 — 폭탄/조커가 손패를 "여러 장
한 번에" 소모한다.** 폭탄은 한 턴에 손패 3장을 동시에 써서, 예를 들어
손패 7장 중 한 번 폭탄을 쓰면 실제로는 7턴이 아니라 5턴만에 손이 빈다.
`h.Count==0` 기반 판정은 이 경우 "5번째 턴"에 예외가 잘못 걸렸다(실제
7번째 턴이 아닌데도 마지막턴 취급) — 사용자가 지적한 "폭탄을 하더라도
마지막턴이 변경되면 안 된다"는 정확히 이 버그를 가리킨 것이었다. 올바른
동작은: 폭탄으로 손이 일찍 비어도 "마지막턴"(7번째) 자체는 그대로이고,
손이 빈 뒤 이어지는 **덱-only 턴들 중 정확히 7번째에서 뒤늦게** 적용돼야
한다 — 손패가 아니라 "이 좌석이 실제로 몇 번째 턴을 밟았는가"를 별도로
세야만 가능한 동작이다.

**구현.**
- `HandSizeLimit => SEATS == 2 ? 10 : 7;`(딜링 손패 장수, `CaptureLine`과
  같은 패턴이지만 완전히 별개 상수) 신규.
- `readonly int[] personalTurnCount = new int[SEATS_MAX];` 신규 —
  `NewGameSeq()`의 기존 per-round 리셋 루프(`goCount[s]=sweeps[s]=...`가
  있는 자리)에서 `personalTurnCount[s]=0`도 같이 리셋.
- **증가 지점 — `PlaySeq`는 "진짜 카드를 내는 지점"(조커 처리 `if`
  블록을 통과한 뒤, `GoStopAudio.CardPlay()` 직전)에서 정확히 1번만
  증가한다.** 조커 자체(`PlayJokerFromHandSeq`)는 턴을 안 끝내고 반드시
  이어서 진짜 카드를 한 장 더 내게 하므로(2026-08-23 확정 설계), 이
  지점에서만 증가시키면 조커+실카드 조합도 자동으로 "1턴"으로 셈해진다.
  폭탄도 이 지점을 딱 한 번만 지나가므로 마찬가지로 "1턴"으로 자동 처리
  — **별도 분기 없이 두 케이스 다 자연스럽게 해결됐다.**
- `DeckOnlySeq`도 자기 턴 시작 시점(`drawPile.Count==0` 조기 종료 이후)에
  동일하게 1번 증가시키고 `isFinalPersonalTurn = personalTurnCount[seat]
  == HandSizeLimit`를 직접 계산 — 예전엔 이 함수가 h.Count==0 시절
  가정("손이 이미 빈 뒤라 항상 예외 대상 아님")을 그대로 믿고
  `ApplyMatchBonus`를 기본값(`allowLastTurnBonus=true`, 항상 허용)으로
  불렀고, 조커 분기는 아예 `false`를 하드코딩해 넘겼다 — 폭탄/조커로
  손이 일찍 빈 케이스에서는 이게 틀린 가정이 되므로, 두 곳 다 실제
  계산값을 넘기도록 고쳤다.
- **`isLastHandCard` 변수명을 `isFinalPersonalTurn`으로 전면 리네임**
  (선언부+`PlaySeq`의 chok/ddadak/sweep 게이트 3곳+`ResolveBonusJoker`의
  매개변수·재귀호출·내부 chok/sweep 게이트) — 손패가 아니라 "이 좌석의
  N번째 개인 턴"이라는 새 의미를 반영, 예전 이름을 그대로 두면 (특히
  DeckOnlySeq에서 참이 되는 경우) 오해의 소지가 있었다. `willDraw`
  기반의 r1(손패쪽) 게이트는 손 안 댔다 — "덱 플립이 이 턴에 아직
  안 끝났으면 r2 결과까지 보고 판단한다"는 그 자체로 독립적인 시퀀싱
  규칙이라 이번 재정의와 무관.

**부수 요청 — "보너스패 다음뒷패를까면 그건 시스템로그로 안남아서
버그인줄알았어. 추가해줘."** `ResolveBonusJoker`의 `var next =
drawPile[0]; drawPile.RemoveAt(0);`(조커의 운명을 정하는 그 카드) 직후에
`AppendChatLine`이 아예 없었다 — `PlaySeq`/`DeckOnlySeq`의 일반 뒷패
로그(`"...뒷패로 {month}월 패가 나왔습니다"`)와 대칭되는 이벤트인데
이 함수만 빠져 있었다. 같은 문구 패턴으로 추가했다.

**검증(Play 모드 라이브, 리플렉션).**
- 정상 카드 1장 재생 — `personalTurnCount[0]` 0→1(동기 구간에서 즉시)
  확인.
- **폭탄(손3+필드1, 5월)** — `personalTurnCount[0]`를 1로 세팅한 뒤
  폭탄 재생 → **1→2로 정확히 +1만**(손패는 3→0으로 동시에 비었지만
  카운터는 안 튐) 확인 — "폭탄을 하더라도 마지막턴이 변경되면 안 된다"는
  요구사항의 핵심 증거.
- **DeckOnlySeq 억제(시나리오 B)** — 손이 이미 빈 상태(`personalTurnCount
  =6`)에서 10월 3장짜리 뻑 무더기(causer=seat1)를 완성시키는 덱-only
  턴을 직접 실행 → `personalTurnCount[0]`가 정확히 7(HandSizeLimit)에
  도달, 캡처(4장)는 정상 진행되지만 seat1의 피는 2장 그대로(스틸
  억제됨) — **h.Count==0 시절엔 절대 재현 불가능했던, "손이 빈 지
  한참 지난 덱-only 턴에서 예외가 뒤늦게 걸리는" 새 동작을 정확히
  확인.**
- **대조군(시나리오 C)** — 같은 구조(8월)를 `personalTurnCount=4→5`
  (한계 미도달)로 재현 → 정상적으로 피 1장 스틸됨(억제 안 됨, 회귀
  없음) 확인.
- **채팅 로그 추가 확인** — 완전 무매칭 손패(4월) 재생 → 뒷패로 조커
  등장 → "나 보너스패 대기" 다음에 **"나님의 뒷패로 2월 패가 나왔습니다"**
  (신규 로그, next=2월 카드) → "나 보너스 획득" 순으로 정확히 채팅
  로그에 기록되는 것 확인.
- 이 세션 전체(여러 차례 재시작 포함) 콘솔 `error`/`exception` 0건
  (Pipeline exec 5초 타임아웃 경고 1건은 이 프로젝트에 이미 문서화된
  무관한 환경 노이즈).

## 고스톱 — SlamIn이 자기 자신을 추적 타겟으로 넘겨 목적지가 시작점
쪽으로 수렴하던 버그 (2026-09-08)

"뒷패에서 보너스패가 나올 때 손패로 낸 카드와 딱 겹쳐 보인다"는 신고를
사용자가 7단계로 정확히 재현해줬다 — "몇 번씩 수정 시도했는데 왜 자꾸
안 맞을까"라는 말대로, 이전 세션들에서 이미 여러 차례 손댔던(오프셋
계산·슬롯 배정 관련) 부분이라 이번엔 데이터가 아니라 **애니메이션
메커니즘 자체**를 의심하고 라이브 리플렉션으로 프레임 단위까지
추적했다.

**1차 진단(그리드/오프셋 계산 자체는 정상)** — `DrawField()`에
임시 진단 로그를 걸어 `field.GroupBy(AssignFieldSlot)`이 만드는
슬롯별 카드 묶음·순서·`FieldStackStep` 계산을 직접 찍어봤다 — anchor는
index 0/offset 0, joker는 index 1/offset `(FieldStackStep(2), -FieldStackStep(2))
= (20,-20)`으로 **정확했다.** 문제는 이 계산이 아니라, 계산된
목적지로 카드를 실제로 "옮기는" 단계에 있다는 뜻이었다.

**2차 진단(순간이동처럼 보이는 착시는 무해한 기존 함정)** — 리플렉션으로
`ResolveBonusJoker`를 직접 호출한 직후(같은 프레임)와 애니메이션이
끝난 뒤(약 1초 후) 각각 슬롯 자식 목록을 찍어보니, 착지 직후 프레임엔
같은 이름의 카드가 2벌(파괴 대기 중인 옛 카드 + 새로 그려진 카드) 잠깐
공존했다 — `Destroy()`가 프레임 끝까지 지연 실행된다는 이 프로젝트의
기존 함정 그대로였고, 1초 뒤엔 정확히 1벌만 남아 무해했다. 여기까지는
버그가 아니었다.

**진짜 원인.** `FlyAndPunch`에는 두 오버로드가 있다 —
`(RectTransform rt, Vector3 from, Vector3 to, ...)`(고정 목적지)와
`(RectTransform rt, Vector3 from, RectTransform target, ...)`(매 프레임
`target.position`을 다시 읽어 "살아있는 목적지"를 정확히 추적하는
용도, 원래는 pos 슬롯 마커처럼 rt와 무관한 별도 Transform을 추적하라고
만든 것). `SlamIn`/`SlamInViaField`의 두 번째 구간이 이 두 번째
오버로드를 부르면서 **`rt` 자기 자신을 `target`으로 넘기고 있었다**
(`FlyAndPunch(rt, fromWorld, rt, ...)`) — `target`이 곧 `rt`라, 루프가
매 프레임 "방금 이 루프 스스로가 옮겨놓은 위치"를 다음 프레임의 목표로
다시 읽는 자기참조 피드백이 생긴다. 매 프레임 보간 비율 `p`가 곱해지며
수렴하는 목표 자체가 시작점(`from`) 쪽으로 계속 줄어들어(`X_n - F =
(∏ᵢ pᵢ)(X_0 - F)`), 의도한 오프셋(예: 20px)의 극히 일부(실측
1.25~1.80px)에서 멈춘다 — 눈으로 보면 시작점 근처 카드와 "딱 겹친"
것처럼 보인다. `grep`으로 확인해보니 이 RectTransform 오버로드는
프로젝트 전체에 정확히 이 두 호출부(`SlamIn`/`SlamInViaField`)뿐이었고,
**둘 다 자기참조로 잘못 쓰고 있었다** — 이 오버로드가 의도한
"진짜 살아있는 타겟 추적" 용도로 쓰인 적은 한 번도 없었다.

**고침.** 두 곳 다 애니메이션 시작 전에 `Vector3 to = rt.position;`
(카드가 `HwatuUI.MakeCard`로 이미 확정해 둔 최종 위치)로 미리
스냅샷해서, 자기참조가 원천적으로 불가능한 `Vector3` 오버로드를
쓰도록 바꿨다 — `SlideHandCard`가 이미 이 패턴(스냅샷 후 고정
목적지로 보간)을 쓰고 있었는데, `SlamIn`만 왜인지 그 패턴을 안
따르고 있었다.

**검증(Play 모드 라이브, 리플렉션).**
- 조커 파킹 시나리오(anchor 카드+joker, 2장 슬롯) — 착지 직후(동기
  구간, 첫 프레임)엔 `(1.80, -1.80)`로 여전히 낮은 값(정상 — 애니메이션
  시작 직후라 아직 목적지에 안 닿은 것뿐, 수정 전 증상과 구분이 안
  되는 시점이라 이것만으로는 판단 불가). **약 1.5초 뒤 재확인** —
  정확히 `(20.00, -20.00)`으로 settled(수정 전엔 이 시점에도 여전히
  1~2px대에 머물러 있었다) — 자기참조 피드백이 제거돼 목적지에
  정확히 도달함을 확인.
- 회귀 확인(일반 플레이 경로) — 실제 `OnPlayerPlay`로 조커 1장(손패
  자체 재생, `PlayJokerFromHandSeq` 경로)과 일반 카드 1장(신규 슬롯
  단독 배치)을 순서대로 재생 — 조커 재생 후 손패 정상 유지(7장,
  자기 자신을 새 카드로 교체), 일반 카드는 새 슬롯에 `(0,0)`
  단독 배치로 정상 착지, 다른 기존 슬롯(2·3·5장 혼합)들도 전부
  올바른 팬아웃 오프셋 유지 — 겹침·순간이동 없음.
- 이 세션 전체(진단 로그 추가/제거 포함 여러 차례 재시작) 콘솔
  `error`/`exception` 0건.

## 고스톱 — 방의 유저 총 머니 변동 합산이 0이 안 되던 버그: 쉬는 좌석의
세션 시작 잔액이 늦게(광팔이 이후) 캡처됨 (2026-09-08)

바로 위 SlamIn 수정 직후 "상단유저는 SetMoneyDelta 호출안되는듯해
안바뀌네"라는 신고를 받고 조사했다가, 실제로는 "호출이 안 된다"가
아니라 다른 문제라는 정정을 받았다 — "그문제가 아닌듯 방의 유저 총
변동급액합산은 0이되어야되는데 안되고있는게 문제야." 라이브 확인
결과 모든 좌석의 `SetMoneyDelta`는 실제로 호출되고 있었지만(스냅샷마다
색 있는 델타 텍스트가 정상 표시), 그 표시값들을 다 더한 합계가
0이어야 하는데(제로섬 게임 — 돈은 좌석 간에 이동할 뿐 생기거나
사라지지 않는다) 그렇지 않은 경우가 있었다.

**원인.** `MoneyDeltaFor(seat)`가 "그 좌석의 상태박스가 처음 그려지는
순간"(=`DrawBadgeStrip`이 그 좌석에 대해 처음 불리는 순간)을 세션
시작 잔액으로 지연 캡처하고 있었다 — 좌석별로 완전히 독립적으로. 그런데
`FillSlot`은 `sittingOutSeat == seat`(쉬는 중)이면 `DrawBadgeStrip`
호출 전에 `return`한다 — **쉬는 좌석은 그 판 내내 한 번도
`MoneyDeltaFor`가 안 불린다.** 그 좌석이 나중에(다음 몇 판 뒤) 실제로
참가하게 돼서야 처음 캡처되는데, 그 사이 광팔이로 이미
`money[seat]`가 움직인 뒤일 수 있다 — 이 경우 캡처되는 "기준선" 자체가
이미 한 번 이체를 받은/낸 뒤의 값이라, 그 좌석의 이후 delta가 실제
누적 변동보다 작게(또는 방향이 어긋나게) 잡혀서 전체 합이 0이 안
됐다. "상단유저"(4인 모드 슬롯2/top)가 눈에 띈 건 실제로 특정 좌석에
치우친 원인이 있어서가 아니라, 광팔이로 쉬는 좌석이 되기 쉬운 자리라
이 시차가 우연히 잘 드러났을 뿐으로 보인다 — 메커니즘 자체는 좌석
무관하게 어느 자리든 걸릴 수 있는 버그였다.

**고침.** `CaptureSessionStartMoneyAllSeats()`(신규) — 아직 캡처
안 된 좌석 전부(다운그레이드로 이어받은 값이 있는 좌석은 건드리지
않음)를 **한 번에** 캡처한다. 이걸 `RefreshStatusBoxIdentitiesBeforeDeal()`
(매 판 딜링 시작 **전**, 광팔이 정산보다도 먼저, 4슬롯 전부에 대해
예외 없이 불리는 기존 훅) 맨 앞에서 호출한다 — 쉬는 좌석이든 아니든
같은 순간에 기준선이 잡히므로, 이후 무슨 이체가 일어나도(광팔이 포함)
합계가 항상 0으로 유지된다. `MoneyDeltaFor`의 좌석별 지연 캡처는
안전망으로 그대로 남겨뒀다(이 훅이 어떤 이유로든 안 불린 경로 —
문서화된 우려사항이었던 네트워크 게스트 등 — 를 위한 폴백, 실제 동작
경로는 이번 수정과 무관하게 안 건드림).

**검증(Play 모드 라이브, 리플렉션).**
- 정상 딜링 직후(광팔이가 자연 발생 안 한 판) — 4좌석 전부
  `sessionStartMoney`가 정확히 그 시점 `money[]`와 일치, 델타 전부
  "변동 없음"으로 정상.
- **버그 재현 조건 직접 구성** — 쉬는 중인 좌석(seat1, 이번 판 아직
  한 번도 `DrawBadgeStrip`을 못 받은 상태)에게 좌석0→좌석1로 20,000원
  합성 이체(실제 광팔이와 동형)를 적용한 뒤 `RebuildUI()` → 전 좌석
  델타 합계 **정확히 0**(수정된 동작이 정상 작동) 확인.
- **옛 방식이 실제로 깨졌었는지 직접 재현** — 같은 이체 직후,
  `sessionStartMoneyCaptured[1]`을 인위적으로 `false`로 되돌려 "옛
  지연 캡처가 이 늦은 시점에 처음 발동했다"는 상황을 재현한 뒤
  `MoneyDeltaFor(1)`을 호출 — 그 순간 기준선이 **이미 이체된 뒤의
  값(520,000)**으로 잘못 캡처되고, 전 좌석 델타 합계가 정확히
  **-20,000**(이체액만큼 어긋남)으로 나와 신고된 증상을 수식적으로도
  정확히 재현했다.
- 오염된 테스트 상태를 원래 값으로 복원 후 합계 0 재확인, 이어서 실제
  `OnPlayerPlay`로 정상 카드 플레이까지 예외 없이 진행되는 것 확인.
- 콘솔 `error`/`exception` 0건(이 세션 전체).

## 고스톱 — 광팔이/첫뻑 등 돈이 움직인 그 순간 즉시 반영되지 않던 문제
(2026-09-08)

바로 위 수정 직후 "광팔고 난 금액은 업데이트 안되네. 게임결과만
업데이트하면 되는게아니고 돈이변할때. 예를들어 광팔기, 첫뻑 등에는
업데이트 계속해줘야될거같아"라는 후속 신고.

**원인.** `money[]`를 직접 바꾸는 중간 이벤트(광팔이 지급,
`ApplyMoneyBonus`가 담당하는 첫뻑/연뻑/첫따닥/첫뻑먹기 판돈 보너스)
전부 그 자리에서 `FlyMoneyFX`(동전 날아가는 연출)만 재생할 뿐, 화면에
실제로 찍힌 금액 텍스트(`moneyText`)나 세션 변동 델타(`countsText`/
`SetMoneyDelta`)는 안 건드리고 있었다 — `RefreshMoneyLabelsOnly()`는
바로 전날(2026-08-25) 세션에 만들어졌지만 그때는 최종 정산(`EndGame`)
끝에서만 불렀다. 그 결과 광팔이·첫뻑 등으로 실제 `money[]`는 바뀌어도
화면 숫자는 다음에 우연히 `RebuildUI()`가 도는 시점(보통 그 좌석의
다음 턴)까지 예전 값에 멈춰 있었다.

**고침.** `RefreshMoneyLabelsOnly()`를 확장해 금액 텍스트뿐 아니라
`SetMoneyDelta`(세션 변동)까지 4슬롯 전부(쉬는 좌석 포함 — money가
바뀌는 데 참가 여부를 가릴 이유가 없다) 같이 갱신하도록 하고,
`money[]`를 직접 바꾸는 두 지점에 그 자리에서 바로 호출을 추가했다 —
광팔이 지급 루프 직후, `ApplyMoneyBonus`의 지급 루프 직후(이 함수
하나가 첫뻑/연뻑/첫따닥/첫뻑먹기 4개 이벤트를 전부 공유하므로 여기
한 곳만 고치면 4곳이 다 해결된다). 최종 정산(`EndGame`)의 기존 호출은
그대로 유지.

**검증(Play 모드 라이브, 리플렉션).**
- 자연 진행 중 실제로 광팔이(`sittingOutWasSqueezed=True`)가 발생한
  판을 그대로 잡아서(내가 강제로 만든 상황이 아니라 진짜 자연
  발생) — **`RebuildUI()`를 한 번도 안 부른 채로** 4슬롯 전부 확인:
  쉬는 좌석(수령자)은 `displayedMoneyText`가 `money[seat]`와 정확히
  일치·`displayedDelta`가 "+4,000원"으로 즉시 반영, 지불자 두 좌석은
  각각 "-2,000원"으로 합계가 정확히 0으로 맞음 — 화면이 그 즉시
  정확했다.
- `ApplyMoneyBonus(0, 9000, "테스트첫뻑비")`를 직접 호출 — 호출 직후
  (별도 RebuildUI 없이) `money[0]`이 정확히 +9000 반영된 값으로,
  `moneyText`/`countsText` 둘 다 그 값과 정확히 일치하는 것 확인.
...
## 고스톱 — 죽은 코드 전수 스윕 (2026-09-12)

"코드랑 주석정리는 끝났는가"라는 질문에, 이전 세션의 "직접 리뷰하며
발견한 것만 치웠다"는 좁은 범위를 벗어나 **정적 분석으로 GoStop 관련
46개 파일 전체를 기계적으로 훑었다** — 메서드 선언을 전부 뽑아 본문
전체에서 몇 번 등장하는지(선언 1회 vs 호출 포함 2회 이상) 세는 방식.

**함정 — 자동 탐지 결과를 그대로 믿으면 안 된다.** 1차 스윕에서
`StartScanningForRooms`/`StepPointPrice`(네트워크 로비 핵심 기능!)가
"호출부 없음"으로 잡혀서 잠깐 심각한 회귀로 오인할 뻔했다 — 원인은
탐지 스크립트의 글롭 범위가 `Assets/Scripts/Games/GoStop/`으로
좁아서, 실제 호출부가 있는 `Assets/Scripts/UI/GoStopNetLobbyUI.cs`
(다른 디렉터리)를 놓친 것이었다. `grep`으로 이 함수명이 GoStop 확장
씬/prefab에 있는지까지 넓혀 재확인하고 나서야 오탐임을 확인했다.
2차 스윕도 `TitleManager.OnGoStop` 등 8개가 "호출부 없음"으로 잡혔는데,
이번엔 진짜 원인이 또 달랐다 — 이 메서드들은 C# 코드에서 직접 안
불리고 **씬 파일(TitleScene.unity)의 직렬화된 UnityEvent(버튼 OnClick)
가 문자열로 가리키는 것**이라, 텍스트 검색 자체가 구조적으로 못 보는
호출 경로였다. `grep -rl`로 씬/prefab YAML까지 검색 범위를 넓혀서야
정확히 판별할 수 있었다. **교훈: "코드에서 안 불린다"고 "안 쓰인다"는
아니다 — Unity는 직렬화된 이벤트 바인딩이라는 별도 호출 경로가 있다.**

**실제로 제거한 죽은 코드 6곳(전부 이 두 단계 검증을 거쳐 확정):**
1. **`HwatuCardArt.cs` 파일 전체 삭제** — 클래스 자체 doc-comment가
   "지금은 1월 광 샘플 하나만 만들어서 퀄리티를 확인하는 단계다.
   통과하면 나머지 11개월 47장을 같은 방식으로 만든다"고 명시한
   프로토타입이었다 — 실제 화투 이미지(Wikimedia Commons SVG 세트,
   `Assets/Resources/Hwatu/`)로 완전히 대체되면서 나머지 47장은 결국
   안 만들어졌고, 이 파일을 참조하는 다른 파일이 프로젝트에 단 하나도
   없었다.
2. **`HwatuShapes.Triangle()` + `triangleCache`** — `HwatuCardArt.
   BuildJanuaryGwang`(소나무 가지)이 유일한 호출부였다. 그 파일이
   삭제되며 완전히 고아가 됐다.
3. **`GoStopNetMessage.ByeMsg()`** — 인자 없는 팩토리. 실제 나가는
   Bye 메시지는 전부 이탈 사유(`text`)를 실어야 해서 두 호출부 다
   `new GoStopNetMessage{type=Bye, text=...}`를 직접 만들지, 이 팩토리를
   쓴 적이 없었다(다른 13개 형제 팩토리는 전부 정상적으로 쓰이고
   있음을 확인 — 이 하나만 시그니처가 실제 요구사항과 안 맞아 고립된
   경우).
4. **`GoStopIcons.Dung()` + `dungCache`** — 뻑(똥) 아이콘을 절차적으로
   그리던 함수. 2026-09-06 세션에 OpenMoji SVG(💩)로 교체되면서 호출부가
   사라졌다(문서에 이미 기록된 마이그레이션인데 옛 구현을 안 지우고
   남겨뒀던 것).
5. **`GoStopIcons.MakeCountBadge()`** — 흔들기/뻑 배지를 매턴 새
   GameObject로 그리던 함수. 2026-08-24 세션에 `GoStopStatusBoxView`
   프리팹의 고정 슬롯(`shakeDots[]`/`ppeokDots[]`) + `SetCountBadge`
   (색만 바꿔 재사용)로 옮겨가며 고립됐다.
6. **`HwatuUI.GroupIntoRows()`** — 피 카드를 "5피씩" 줄로 묶던 단순
   그리디 알고리즘. 2026-09-03 세션들에서 `FillCapZone`이 "줄이 정확히
   weight-4로 찼는데 다음 카드가 쌍피라 넘칠 때 마지막 홑피를 다음
   줄로 미루는" 스왑 로직까지 필요해지며 더 정교한 패킹을 인라인으로
   직접 구현했고, 그 뒤로 이 범용 헬퍼를 부르는 곳이 없어졌다.

**의도적으로 안 건드린 것 — `TitleManager.OnContinue`/`OnOptions`/
`OnExit`.** 이 셋도 씬 YAML에서 검색해봤지만 정말로 어디에도(씬·
prefab 어디에도) 안 걸려 있었다(`continueButton: {fileID: 0}`,
`optionsPanel: {fileID: 0}` — 참조 자체가 비어 있음, Inspector에서
할당된 적이 없다는 뜻). 그런데 이건 위 6곳과 성격이 다르다 — **버려진
프로토타입이 아니라 절반만 만들어진 기능으로 보인다.** `OnContinue`가
쓰는 `SaveManager`(이어하기 저장/불러오기)는 `GameSceneManager.cs`/
`ColorSortGame.cs`에서 여전히 활발히 쓰이고 있다 — 저장 시스템 자체는
살아있는데 타이틀 화면에 "이어하기" 버튼을 실제로 만들어 연결하는
마무리 작업만 안 끝난 것으로 보인다. 8개 형제 메서드(`OnGoStop` 등)는
전부 정상적으로 씬에 연결돼 있어서 이 3개만 유독 빠진 게 우연이라고
보기 어렵다 — **삭제하지 않고 그대로 뒀다.** 이어하기/설정/종료 버튼을
마저 완성할 계획이 있으면 이 셋을 그대로 쓰면 되고, 정말 필요 없는
기능이면 다음에 명시적으로 요청하면 지운다.

**검증(컴파일 + Play 모드 라이브).** 6곳 제거 후 `unity command
recompile` 클린(0 에러) 확인. 라이브로 4인 게임을 새로 딜링하고 실제
`OnPlayerPlay`로 카드를 재생(우연히 조커 카드가 걸려 조커 처리
경로까지 같이 확인됨) — 콘솔 `error`/`exception` 0건. `GoStopIcons.
Bell()`/`Bomb()`(같은 파일에서 `Dung`/`MakeCountBadge`와 나란히
있던 형제 함수들, 삭제 작업이 잘못 건드리지 않았는지 직접 재검증)와
`HwatuShapes.RoundedRect()`/`Circle()`(`Triangle` 삭제가 같은 파일의
다른 캐시 필드를 안 건드렸는지 확인)를 각각 직접 호출해 정상 동작
확인.

**최종 답 — "코드랑 주석정리는 끝났는가"에 대해:** 이번 정적 분석
스윕으로 GoStop 관련 46개 파일의 죽은 메서드는 전수 확인했고(호출부
없는 6개를 찾아 전부 제거, 나머지는 씬 바인딩으로 살아있음을 확인),
남은 3개(`OnContinue`/`OnOptions`/`OnExit`)는 죽은 게 아니라 미완성
기능이라 남겨뒀다. **다만 이 스윕은 "메서드/필드가 호출되는가"만
검사했다** — 문법적으로 옳지만 의미상 스테일한 주석(설명이 실제 동작과
어긋난 문서)까지 전부 훑은 건 아니다(그런 사례를 발견하면 그때그때
고쳐왔다 — `BuildInfoBlock` 위 3중 stale 주석 등). 로그 정리는 이전
